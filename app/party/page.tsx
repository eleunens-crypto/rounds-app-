  ...
  if (view === "start") {
    return (
      <div style={{ ...S.page, minHeight: "auto", padding: "0 0 40px" }}><div style={{ ...S.wrap, paddingTop: "calc(env(safe-area-inset-top, 0px) + 70px)" }}>
        ...
        <div style={{ background: RAND, borderRadius: 16, padding: "16px 14px", marginBottom: 15, textAlign: "center" }}>
          <RundoLogo size={62} />
          <div style={{ display: "inline-flex", alignItems: "center", gap: 9, marginTop: 9 }}>
            <KlinkIcoon size={30} />
            <span style={{ fontSize: 16, color: "#c3cbd8", lineHeight: 1.4, textAlign: "left" }}>{L.tagline}</span>
          </div>
        </div>
        <div style={{ textAlign: "center", fontSize: 21, fontWeight: 800, color: "#16203a", marginBottom: 13 }}>{L.chooseHow}</div>
          ...
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", background: "#fff", border: bpSettle === false ? `2px solid ${MODUS_SNEL.rand}` : `1px solid rgba(29,41,66,0.16)`,
              ...
              <button onClick={() => setBpSettle(false)}
                style={{ position: "relative", width: "100%", display: "block", textAlign: "center", padding: "11px 14px 12px", cursor: "pointer", border: "none",
                ...
                <span style={{ display: "flex", justifyContent: "center", marginTop: 6, marginBottom: 7 }}>
                  <NoteerIcoon size={64} kleur={MODUS_SNEL.rand} />
                </span>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", background: "#fff", border: bpSettle === true ? `2px solid ${MODUS_FAIR.rand}` : `1.5px solid ${MODUS_FAIR.randZacht}`,
              ...
              <button onClick={() => { setBpSettle(true); setModeInfo((m) => m === "fair" ? m : null) }}
                style={{ position: "relative", width: "100%", display: "block", textAlign: "center", padding: "11px 14px 12px", border: "none", cursor: "pointer",
                ...
                <div style={{ marginTop: 4 }}>
                  ...
                  {potSpent > 0.005 && (
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, fontSize: 15.5, padding: "3px 0" }}>
                      <span style={{ color: "#2f5693", fontWeight: 700, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><ZakjeIcoon size={15} /> {L.fromPot}{potNrs.length > 0 && <> · {L.inRounds(potNrs.join(", "))}</>}</span>
        ...
        <div style={{ ...S.card, background: "#f2f6fc", border: potEdit !== null ? "1.5px solid rgba(47,111,181,0.5)" : "1px solid rgba(47,111,181,0.35)" }}>
          {potEdit === null ? (
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
              <span style={{ fontSize: 15.5, color: "#2f5693", minWidth: 0 }}><ZakjeIcoon size={15} /> <b>{L.potTitel} {euro(potContribTotal)}</b> · {potZonderNamen ? L.potEvenIn : L.potIn}: {people.map((p) => `${p.name} ${euro(contribOf(p.id))}`).join(" · ")}</span>
            ...
            return (
              <div>
                <div style={{ fontSize: 16, fontWeight: 800, color: "#2f5693", marginBottom: 10 }}><ZakjeIcoon size={15} /> {L.potWhoFrom(euro(potContribTotal))}</div>
                {people.map((p) => (
