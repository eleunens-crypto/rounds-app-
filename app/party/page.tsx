...
// leaveWaitFor: id van een gast die wil vertrekken en op het bedrag van dit rondje
// wacht. Zet de haler en de beheerder een seintje; verdwijnt zodra het bedrag er is.
type Round = { id: string; seq: number; status: "open" | "pending" | "closed"; orders: Assign; anon: Anon; payers: Record<string, number>; amount: number; potPart: number; gaveBack: Record<string, number>; members: string[]; startedBy: string | null; proposal: Proposal; headcount: number; leaveWaitFor?: string | null }
...
const T = {
  nl: {
    ...
    viaQrTitle: "Scande zelf de QR-code",
    howPaid: "Hoe betaald?",
    paidSelfShort: "💶 Zelf",
    ...
    orderLockedSub: (naam: string) => `${naam} gaat ze halen. Aanpassen kan niet meer.`,
    ...
    repeatRoundSub: "exact hetzelfde of aangepast",
    ...
    yourChoiceFixed: "Jouw keuze staat vast",
    passedOnTo: (n: string) => `Doorgegeven aan ${n}`,
    ...
    guestSeeSettlement: "Afrekening bekijken",
    youPayStill: "Jij betaalt nog",
    ...
    guestSettlementTitle: "Afrekening",
    guestSettlementIncomplete: (n: number) => `Nog niet volledig. ${n} rondje${n === 1 ? "" : "s"} ${n === 1 ? "heeft" : "hebben"} nog geen bedrag — de cijfers kunnen nog wijzigen.`,
    getsFromGuest: (v: string, naam: string) => `krijgt ${v} van ${naam}`,
    paysToGuest: (v: string, naam: string) => `betaalt ${v} aan ${naam}`,
    allSquareGuest: "niets meer te verrekenen",
    imLeaving: "Ik ga weg",
    handoverTitle: "Beheer overdragen",
    handoverSub: "Alleen wie al gescand heeft. Jij wordt daarna gewoon gast.",
    handoverTo: (naam: string) => `Overdragen aan ${naam}`,
    handoverNotScanned: "niet gescand",
    handoverOfferTitle: (naam: string) => `${naam} geeft jou het beheer`,
    handoverOfferBody: "Jij vult dan bedragen in en rekent af.",
    handoverAccept: "Ok, ik neem het over",
    handoverDecline: "Liever niet",
    handoverDone: (naam: string) => `${naam} beheert de groep nu.`,
    handoverDeclined: (naam: string) => `${naam} nam het beheer niet over.`,
    leaveConfirmTitle: "Afgerekend, ik ga",
    leaveAskTitle: "Zeker dat je weggaat?",
    ...
    leaveMissingBody: (seq: number) => `Rondje ${seq} heeft nog geen bedrag, en jij nam daar iets. Zonder dat bedrag klopt jouw stand niet.`,
    leaveWaitingSent: (namen: string) => `Seintje gestuurd naar ${namen}`,
    ...
    adminLeaveBanner: (naam: string, seq: number) => `${naam} wil naar huis · wacht op rondje ${seq}`,
    adminLeaveBannerBody: "Vul het bedrag in, dan kan die persoon afrekenen.",
    fillAmountForLeaveBtn: (seq: number) => `Bedrag rondje ${seq} invullen`,
    ...
    paidPotHint: "ⓘ later eerlijk verrekend volgens wie wat dronk",
    potLeftShort: (v: string) => `nog ${v}`,
    potShortNote: (pot: string, rest: string) => `Er zit nog ${pot} in de pot. De resterende ${rest} staat op jou als voorgeschoten.`,
    ...
    allDrinksInRound: "Alles staat er al in.",
    adjustLower: "aanpassen",
    noNamePill: (n: number) => `${n} zonder naam`,
    noAmountPill: "geen bedrag",
    completePill: "✓ compleet",
  ...
  fr: {
    ...
    viaQrTitle: "A scanné le QR lui-même",
    ...
    orderLockedSub: (naam: string) => `${naam} va les chercher. Plus moyen de changer.`,
    ...
    repeatRoundSub: "pareil ou ajust\u00e9e",
    ...
    guestSeeSettlement: "Voir le décompte",
    youPayStill: "Tu dois encore",
    ...
    guestSettlementTitle: "Décompte",
    guestSettlementIncomplete: (n: number) => `Pas encore complet. ${n} tournée${n === 1 ? "" : "s"} sans montant — les chiffres peuvent encore changer.`,
    getsFromGuest: (v: string, naam: string) => `reçoit ${v} de ${naam}`,
    paysToGuest: (v: string, naam: string) => `doit ${v} à ${naam}`,
    allSquareGuest: "rien à régler",
    imLeaving: "Je m'en vais",
    handoverTitle: "Transf\u00e9rer la gestion",
    handoverSub: "Uniquement ceux qui ont scann\u00e9. Tu deviens ensuite un invit\u00e9.",
    handoverTo: (naam: string) => `Transf\u00e9rer \u00e0 ${naam}`,
    handoverNotScanned: "pas scann\u00e9",
    handoverOfferTitle: (naam: string) => `${naam} te confie la gestion`,
    handoverOfferBody: "Tu pourras saisir les montants et faire le d\u00e9compte.",
    handoverAccept: "D'accord, je reprends",
    handoverDecline: "Plut\u00f4t pas",
    handoverDone: (naam: string) => `${naam} g\u00e8re le groupe maintenant.`,
    handoverDeclined: (naam: string) => `${naam} n'a pas repris la gestion.`,
    leaveConfirmTitle: "Réglé, je pars",
    leaveAskTitle: "Tu pars vraiment ?",
    ...
    leaveMissingBody: (seq: number) => `La tournée ${seq} n'a pas encore de montant, et tu y as pris quelque chose.`,
    leaveWaitingSent: (namen: string) => `Signal envoyé à ${namen}`,
    ...
    adminLeaveBanner: (naam: string, seq: number) => `${naam} veut rentrer · attend la tournée ${seq}`,
    adminLeaveBannerBody: "Indique le montant pour que cette personne puisse régler.",
    fillAmountForLeaveBtn: (seq: number) => `Montant tournée ${seq}`,
    ...
    paidPotHint: "ⓘ réparti équitablement selon ce que chacun a bu",
    potLeftShort: (v: string) => `reste ${v}`,
    potShortNote: (pot: string, rest: string) => `Il reste ${pot} dans la cagnotte. Les ${rest} restants sont avancés par toi.`,
    ...
    allDrinksInRound: "Tout y est d\u00e9j\u00e0.",
    adjustLower: "modifier",
    noNamePill: (n: number) => `${n} sans nom`,
    noAmountPill: "sans montant",
    completePill: "\u2713 complet",
...
export default function PartyTest() {
  ...
  const [view, setView] = useState<"start" | "setup" | "settings" | "order" | "confirmed" | "hub" | "final" | "quickSettle" | "fairIntro" | "fairSetup" | "roundsOverview" | "fairAssign" | "payers">("start")
  ...
  // "Zelfde opnieuw" in Neem zelf op: eerst het barlijstje van het vorige rondje, met
  // plus en min. Pas daarna kies je: meteen bestellen, of verder aanpassen op het
  ...
  // Aan wie biedt de beheerder het beheer aan? Het id van een pláats, niet van een
  // toestel: pas als die persoon aanvaardt, gaat owner_id naar háár toestel. Zo landt
  ...
  // Wie er nu nog is. Alles wat over het lópende of een nieuw rondje gaat — namenkiezer,
  // wie-moet-nog-kiezen, de haler — rekent hiermee. De volledige `people` blijft in
  // gebruik voor oude rondjes en de eindbalans, want een vertrokken gast dronk wél mee.
  const nogAanwezig = people.filter((p) => !p.left)
  ...
  const [editDraft, setEditDraft] = useState<{ drinks: Record<string, number>; amount: number; headcount: number; payers: string[]; usePot: boolean; weg: Record<string, string[]> } | null>(null)
  ...
  const startEditRound = (r: Round) => {
    ...
    setEditDraft({ drinks: d, amount: r.amount || 0, headcount: Math.max(1, r.headcount || 1),
      ...
      usePot: (r.potPart || 0) > 0.005, weg: {} })
    setEditRoundId(r.id)
  }
  const cancelEditRound = () => { setEditDraft(null); setEditRoundId(null); setAddDrinkOpen(false); setKiesWeg(null) }
  ...
  const saveEditRound = async (r: Round) => {
    ...
    Object.entries(editDraft.drinks).forEach(([did, n]) => {
      ...
      if (delta > 0) rBumpAnon(idx, did, delta)
        ...
        // Per gast optellen en één keer wegschrijven: rBump geeft een delta door aan de
        // server met de rónde uit de vorige render, dus twee losse aanroepen voor dezelfde
        // gast zouden op een verouderd tussenstandje rekenen.
        const lijst = (editDraft.weg[did] ?? []).slice(0, -delta)
        const perGast: Record<string, number> = {}
        lijst.forEach((pid) => { perGast[pid] = (perGast[pid] ?? 0) + 1 })
        Object.entries(perGast).forEach(([pid, aantal]) => {
          if (pid === "") rBumpAnon(idx, did, -aantal)
          else rBump(idx, did, pid, -aantal)
        })
        // Meer weggehaald dan we onthielden? Dan vult de oude regel de rest aan.
        const rest = -delta - lijst.length
        if (rest > 0) rRemoveUnits(idx, did, rest)
      }
    })
    ...
    if (editDraft.amount > 0.005 && r.leaveWaitFor) clearLeaveWait(r.id)
    cancelEditRound()
  }
  ...
  // Pas als we de URL gelezen hebben, weten we of het welkomscherm mag. Tot dan tonen we
  // hieronder een leeg vlak in plaats van het scherm alvast te tekenen — anders flitst het
  // één frame in beeld bij wie via de kiezer binnenkomt.
  ...
  const gastRondjeKop = useRef<HTMLDivElement | null>(null)
  // De kaart van wie het rondje haalt. Na het starten scrollen we hierheen, zodat je
  // meteen ziet wie er al koos in plaats van naar de kop van de pagina te kijken.
  const halerKop = useRef<HTMLDivElement | null>(null)
  const naarHalerKop = (herkans = true) => requestAnimationFrame(() => {
    const el = halerKop.current
    if (!el) { if (herkans) setTimeout(() => naarHalerKop(false), 150); return }
  ...
  // Meldingen die via het live-kanaal binnenkomen zijn nieuws, geen vraag: ze horen
  ...
  useEffect(() => {
    ...
    if (weg && meId && rounds.every((r) => r.id !== weg.id)) {
      ...
      void (async () => {
        ...
        // Bestaat het rondje nog, dan is het afgerond en niet geannuleerd. Dat is het
        // moment waarop de rest het mag weten: pas nu ligt de bestelling vast.
        if (data) { setGastBalk(weg.door); return }
        setNotice(L.roundCancelled(weg.door))
      })()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [openRoundId])
  ...
  useEffect(() => {
    ...
    // Alleen de starter hoort dit: hij is de enige die er iets mee moet. De rest verneemt
    // het pas wanneer hij echt afrondt, anders denkt iedereen dat het al beslist is.
    // Een balk boven de kaart, geen venster eroverheen: de knop eronder moet meteen
    // aantikbaar zijn.
    if (ikHaalNu) setStarterBalk(true)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Is de avond afgesloten? Dan staan de bedragen vast en toont elk scherm bovenaan de
  // weg naar de eindafrekening.
  const [groepAf, setGroepAf] = useState(false)
  ...
  const [gastBalk, setGastBalk] = useState<string | null>(null)
  useEffect(() => {
    if (!starterBalk) return
    const t = setTimeout(() => setStarterBalk(false), 8000)
    return () => clearTimeout(t)
  }, [starterBalk])
  useEffect(() => {
    if (!gastBalk) return
    const t = setTimeout(() => setGastBalk(null), 8000)
    return () => clearTimeout(t)
  }, [gastBalk])
  // Wijzigt iemand na zijn bevestiging, dan zakt de groep terug onder volledig. De balk
  // klopt dan niet meer en moet meteen weg. We wissen ook het geheugen van de melding,
  ...
  useEffect(() => {
    ...
    setAllenGemeld((g) => (g === openRoundId ? null : g))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [alGekozen, people.length, openRoundId])
  ...
  // groen = ik bevestigde mijn keuze; mijnTekst = wat ik nam, om in de kop te tonen.
  // De twee balken. Ze horen boven de kaart, want ze gaan over wat er in de kaart staat.
  const renderBalk = (kleur: "groen" | "blauw", tekst: React.ReactNode, sluit: () => void) => {
    const g = kleur === "groen"
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 9, borderRadius: 11, padding: "10px 12px", marginBottom: 10,
        background: g ? "rgba(31,138,76,0.12)" : "rgba(13,124,140,0.12)",
        border: `1px solid ${g ? "rgba(31,138,76,0.4)" : "rgba(13,124,140,0.4)"}` }}>
        <span style={{ fontSize: 14, lineHeight: 1.4, color: g ? "#1f6b3a" : MODUS_FAIR.tekst }}>{tekst}</span>
        <button onClick={sluit} aria-label={L.closeWord}
          style={{ marginLeft: "auto", flexShrink: 0, background: "none", border: "none", cursor: "pointer", fontSize: 16, lineHeight: 1, color: g ? "#1f6b3a" : MODUS_FAIR.tekst, padding: 2 }}>✕</button>
      </div>
    )
  }
  ...
  const balken = () => (
    <>
      {starterBalk && renderBalk("groen", <><b style={{ fontWeight: 800 }}>{L.allChoseShort}</b> {L.allChoseCta}</>, () => setStarterBalk(false))}
      {gastBalk && renderBalk("blauw", <><b style={{ fontWeight: 800 }}>{L.orderLocked}</b> {L.orderLockedSub(gastBalk)}</>, () => setGastBalk(null))}
    </>
  )
  const renderRunnerBar = (compact = false, groen = false, mijnTekst = "") => {
    ...
    if (!openRoundId && haalInfo) {
      return (
        <>{balken()}<div style={{ ...S.card, background: "#fff", border: `2px solid ${MODUS_FAIR.rand}`, boxShadow: `0 6px 18px -12px ${MODUS_FAIR.gloed}` }}>
    ...
    if (ikHaal) {
      // Bevestigd betekent bevestigd: alleen wie op de knop tikte telt mee. Vroeger
      // telde deze regel ook wie enkel drankjes had aangetikt, waardoor de teller
      // opliep terwijl de rij eronder nog "bezig met kiezen" zei.
      const klaar = nogAanwezig.filter((pp) => isKlaar(pp.id))
      const allen = klaar.length >= nogAanwezig.length && nogAanwezig.length > 0
      return (
        <>{balken()}<div ref={halerKop} style={{ ...S.card, position: "sticky", top: 0, zIndex: 9, scrollMarginTop: 0, background: "#fff", border: `2px solid ${MODUS_FAIR.rand}`, boxShadow: `0 8px 20px -12px ${MODUS_FAIR.gloed}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 9 }}>
            ...
            <span style={{ minWidth: 0 }}>
              ...
              <span style={{ display: "block", fontSize: 14, color: allen ? "#1f6b3a" : "#5a8f99", fontWeight: allen ? 800 : 400, marginTop: 1 }}>{allen ? `✓ ${L.allChose}` : L.confirmedOf(klaar.length, nogAanwezig.length)}</span>
            </span>
          </div>
          ...
          {wieOpen && nogAanwezig.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 8 }}>
              {nogAanwezig.map((pp) => {
                ...
                const ok = isKlaar(pp.id)
                return (
                  <span key={pp.id} style={{ fontSize: 12.5, fontWeight: 800, padding: "4px 9px", borderRadius: 999,
                    background: slaOver ? "rgba(29,41,66,0.08)" : ok ? "rgba(31,138,76,0.14)" : "rgba(240,165,0,0.16)",
                    color: slaOver ? "#6b7484" : ok ? "#1f6b3a" : "#8a5e0f" }}>
                    {slaOver ? "—" : ok ? "✓" : "⏳"} {pp.id === meId ? L.youWord : pp.name}
                  </span>
                )
              })}
            </div>
          ...
          <div style={{ marginBottom: 10, display: !wieOpen || (nogAanwezig.length > 4 && !lijstOpen) ? "none" : "block" }}>
            {nogAanwezig.map((pp, pi) => {
              ...
              return (
                <div key={pp.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10, fontSize: 16, padding: "6px 0", borderBottom: pi < people.length - 1 ? `1px solid ${MODUS_FAIR.lijnZacht}` : "none" }}>
                  <span style={{ flexShrink: 0, fontWeight: isOk ? 800 : 600, color: slaOver ? "#8b93a3" : isOk ? "#1f6b3a" : "#8a5e0f", maxWidth: "48%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    <span style={{ display: "inline-block", width: 16 }}>{slaOver ? "—" : isOk ? "✓" : "⏳"}</span>{pp.name}{pp.id === meId ? ` (${L.youWord})` : ""}
                  </span>
                  {/* Ook zonder bevestiging tonen we wat er al in het mandje zit: dat is
                      wat je aan de toog gaat vragen. De toevoeging erachter zegt dat het
                      nog kan wijzigen. */}
            ...
            {/* Wat je straks aan de toog moet vragen, alvast opgeteld. De drankjes zelf
                staan al in de lijst hierboven; hier volstaat het aantal. */}
            {barTotalen().length > 0 && (
              <div style={{ background: MODUS_FAIR.tint, borderRadius: 10, padding: "8px 11px", marginTop: 9, fontSize: 15, color: MODUS_FAIR.tekst, lineHeight: 1.45 }}>
                <b>{L.togetherDrinks(barTotalen().reduce((a, b) => a + b.n, 0))}</b>
              </div>
            )}
          </div>
          ...
          {/* Koos en bevestigde iedereen, dan is dit de enige zinnige tik die er nog is.
              De knop kleurt groen en pulseert zachtjes, zodat de starter niet hoeft te
              zoeken. Zodra er iemand terugvalt naar "bezig", is hij weer gewoon turkoois. */}
          <style>{`@keyframes rundoKlaarGloed{0%,100%{box-shadow:0 0 0 0 rgba(31,138,76,0.5)}50%{box-shadow:0 0 0 9px rgba(31,138,76,0)}}@media (prefers-reduced-motion: reduce){.rundo-klaar{animation:none!important;box-shadow:0 0 0 4px rgba(31,138,76,0.3)}}`}</style>
    ...
    if (startedBy) {
      ...
      return (
        <>{balken()}<div style={{ ...S.card, position: "sticky", top: 0, zIndex: 9, background: groen ? "#f4fbf6" : "#fff", border: `2px solid ${groen ? "#1f8a4c" : MODUS_FAIR.rand}`, boxShadow: `0 8px 20px -12px ${MODUS_FAIR.gloed}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 10 }}>
            ...
            <span style={{ flexShrink: 0, width: 38, height: 38, borderRadius: "50%", background: groen ? "#1f8a4c" : MODUS_FAIR.knop, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: groen ? 21 : 18, fontWeight: 800 }}>{groen ? "✓" : (runnerName() || "?").charAt(0).toUpperCase()}</span>
            <span style={{ minWidth: 0 }}>
              <span style={{ display: "block", fontSize: 17.5, fontWeight: 800, color: groen ? "#1f6b3a" : MODUS_FAIR.tekst }}>{L.roundBusyX(runnerName())}</span>
          ...
          {groen && !!mijnTekst && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, paddingTop: 9, marginTop: -2, marginBottom: 2, borderTop: "1px solid rgba(31,138,76,0.22)" }}>
              <span style={{ background: "#fff", border: "1px solid rgba(31,138,76,0.35)", borderRadius: 999, padding: "4px 11px", fontSize: 13, fontWeight: 800, color: "#1f6b3a" }}>{mijnTekst}</span>
            </div>
          )}
          {/* Wat er van jou verwacht wordt, met de uitweg ernaast. Koos je al iets, dan is
              die uitweg niet meer nodig. */}
          {/* In compacte modus staan deze twee knoppen in de zwevende onderbalk, binnen
              duimbereik. Ze hier herhalen zou dezelfde vraag twee keer stellen. */}
          {meId && !compact && (
  ...
  // Beheer overdragen. De beheerder biedt aan (stap 1), de ontvanger aanvaardt op zijn
  // eigen toestel (stap 2). Pas dan verhuist owner_id — anders zou een groep stuurloos
  // achterblijven bij iemand die niet kijkt.
  const offerHandover = (personId: string) => {
    if (!groupId) return
    setHandoverTo(personId)
    supabase.from("party_groups").update({ handover_to: personId }).eq("id", groupId).then(({ error }) => {
      if (error) { setNotice("Overdragen mislukt: " + error.message); setHandoverTo(null) }
    })
  ...
  const acceptHandover = async () => {
    ...
    if (error) { setNotice("Overnemen mislukt: " + error.message); return }
    setOwnerDevice(me.current); setHandoverTo(null)
  }
  const declineHandover = () => {
    if (!groupId) return
    setHandoverTo(null)
    supabase.from("party_groups").update({ handover_to: null }).eq("id", groupId)
  }
  ...
  const loadParty = useCallback(async (gid: string) => {
    ...
    if (g) {
      ...
      setHandoverTo((g.handover_to ?? null) as string | null)
      setGroepAf(!!g.finalized)
    ...
    setPeople((vorige) => (pp || []).map((r, i: number) => {
      ...
      const volgnr = i + 1
      // Staat er op de server toch een standaardnaam als "Gast 8", dan telt die als
      // naamloos: anders blijft het oude nummer staan na het wissen van gasten.
      const ruweNaam = (r.name || "").trim()
      const serverNaam = isGuestDefault(ruweNaam) ? "" : ruweNaam
    ...
    const alle = (rr || []).map((r) => ({
      ...
      leaveWaitFor: (r.leave_wait_for ?? null) as string | null,
    }))
    ...
    ;(async () => {
      ...
      if (sessie?.g) {
        ...
        if (data) {
          ...
          const afrekenen = sessie.v === "payers" || sessie.v === "fairAssign" || sessie.v === "final" || sessie.v === "fairSetup" || sessie.v === "quickSettle" || sessie.v === "settings"
  ...
  useEffect(() => {
    ...
    const maakKanaal = () => {
      ...
      c.on("broadcast", { event: "melding" }, (msg) => { const t = (msg as { payload?: { tekst?: string } }).payload?.tekst; if (t) setVluchtig(t) })
      c.on("broadcast", { event: "voorJou" }, (msg) => {
        const p = (msg as { payload?: { voor?: string; tekst?: string } }).payload
        if (p?.tekst && p.voor && p.voor === meIdRef.current) setVluchtig(p.tekst)
      })
  ...
  const naarDrankjes = useCallback(() => {
    ...
    // Even wachten tot het scherm getekend is, dan naar het rondjeblok scrollen. De kop
    // met logo en groepsnaam schuift zo uit beeld en je begint meteen bij de drankjes.
  ...
  const goHub = () => { const to = () => { setOpenRound(rounds.length - 1); setView("hub") }; if (view === "confirmed") setConfirmDlg({ variant: "danger", msg: L.unfinishedWarn, yes: L.leaveAnyway, onYes: () => { setConfirmDlg(null); dropUnpaidRound(); to() } }); else to() }
  ...
  const goQuickSettle = () => {
    ...
    if (!settle && opNaam && unassignedAllRounds > 0) {
      ...
      if (fr >= 0) { settleNaToewijzen.current = true; setOpenRound(fr); setAllRoundsOpen(false); setAssignAllMode(true); setAssignIdx(fr); setView("hub") }
      return
    }
  ...
  const confirmFairSetup = async () => {
    ...
    // Stap 1 leidt altijd naar stap 2 zélf, ook als alles al toegewezen is: dan zie je
    // gewoon dat het klaar is. De volgorde: wie was erbij, wie dronk wat (stap 2), wat
    // kostte het en wie betaalde (stap 3). Eerder kwamen de bedragen vóór het toewijzen;
    // nu eerst wie wat dronk (stap 2), dan wat het kostte en
    // wie betaalde (stap 3). Wie wat dronk kan je aan tafel meteen aantikken; het bedrag
    // zoek je op het bonnetje, en dat hoort bij het betalen.
    if (fromQuick && rounds.length > 0) {
      setFillMode(false); setAssignAllMode(false); setAssignIdx(null)
      setView("fairAssign")
      return
    }
    setView("hub")
  ...
  const goAssignUnassigned = () => {
    ...
    if (fr < 0) return
    setOpenRound(fr); setAllRoundsOpen(false); setView("hub"); setAssignIdx(fr)
  }
  ...
  // zowel de haler als de beheerder zodra hun scherm ververst, via dezelfde realtime-lijn
  // als de rest van het rondje.
  const setLeaveWait = (roundId: string, personId: string) => {
    setRounds((rs) => rs.map((rr) => rr.id === roundId ? { ...rr, leaveWaitFor: personId } : rr))
    supabase.from("party_rounds").update({ leave_wait_for: personId }).eq("id", roundId)
  }
  const clearLeaveWait = (roundId: string) => {
    setRounds((rs) => rs.map((rr) => rr.id === roundId ? { ...rr, leaveWaitFor: null } : rr))
    supabase.from("party_rounds").update({ leave_wait_for: null }).eq("id", roundId)
  }
  const goFinal = () => {
  ...
  // De bestelling van het vorige rondje, beperkt tot wie er nog is.
  const vorigeBestelling = (last: Round) => {
    const orders: Assign = {}
  ...
  // Het barlijstje zegt alleen hoeveel van elk drankje; de bestelling weet ook bij wie.
  // Wat erbij komt, staat nog zonder naam. Wat eraf gaat, komt eerst uit wat zonder
  // naam stond en pas daarna bij wie er het meest van had, zoals rRemoveUnits dat doet.
  const naarTotalen = (orders: Assign, anon: Anon, totalen: Record<string, number>) => {
    const o: Assign = {}
    const a: Anon = {}
    const ids = new Set([...Object.keys(orders), ...Object.keys(anon), ...Object.keys(totalen)])
    ids.forEach((did) => {
      const doel = Math.max(0, totalen[did] ?? 0)
      const rij: Record<string, number> = { ...(orders[did] || {}) }
      let los = anon[did] || 0
      const nu = los + Object.values(rij).reduce((s, q) => s + (q || 0), 0)
      if (doel > nu) los += doel - nu
      else {
        let weg = nu - doel
        const uitLos = Math.min(weg, los)
        los -= uitLos; weg -= uitLos
        for (const [pid] of Object.entries(rij).sort((x, y) => y[1] - x[1])) {
          if (weg <= 0) break
          const k = Math.min(weg, rij[pid])
          rij[pid] -= k; weg -= k
        }
      }
      const schoon = Object.fromEntries(Object.entries(rij).filter(([, q]) => q > 0))
      if (Object.keys(schoon).length) o[did] = schoon
      if (los > 0) a[did] = los
    })
    return { orders: o, anon: a }
  }
  // Alle drankjes in één keer in het rondje zetten. Bestaat party_bump_many nog niet in
  // de databank, dan één voor één, zodat een oudere installatie blijft werken.
  const zetInRondje = async (rid: string, orders: Assign, anon: Anon) => {
    if (!groupId) return
    const items: { person: string | null; drink: string; delta: number }[] = []
  ...
  const bestelOpnieuw = async (totalen: Record<string, number>, bronIdx?: number) => {
    ...
    const bron = (gekozen !== null && rounds[gekozen]) || last
    ...
    try {
      ...
      const rid = await ensureRound(meId ?? null)
      if (!rid) return
      await zetInRondje(rid, orders, anon)
      const leden = people.map((p) => p.id)
      // Zelfde regel als commitRound: het aantal personen van het vorige rondje blijft staan.
      const headcountNu = opNaam === true ? leden.length : Math.max(1, last.headcount || 1)
      const { error } = await supabase.from("party_rounds")
        .update({ status: "closed", gave_back: {}, members: leden, headcount: headcountNu, closed_at: new Date().toISOString() })
        .eq("id", rid)
      if (error) { setNotice("Rondje bevestigen mislukt: " + error.message); return }
      setOpenRoundId(null); setStartedBy(null)
  ...
  const repeatRound = (totalen?: Record<string, number>, bronIdx?: number) => {
    ...
    const { orders, anon } = totalen ? naarTotalen(basis.orders, basis.anon, totalen) : basis
    setRoundNr(rounds.length + 1)
    ...
    void (async () => {
      ...
      // één vergrendeling in plaats van tien.
      await zetInRondje(rid, orders, anon)
      await openAntwoordveld(rid)
      loadParty(groupId)
    })()
  ...
  const renderDialogs = () => (
    <>
          ...
          return (
            <div style={S.overlay} onClick={() => { settleNaToewijzen.current = false; setAssignIdx(null); setAssignAllMode(false) }}>
              <div style={{ ...S.sheet, maxHeight: "86vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
                ...
                {/* Ook hier kan je stoppen, met dezelfde vraag als op stap 1 en 2. */}
                {fromQuick && !settle && (
                  <button onClick={() => { setAssignIdx(null); setAssignAllMode(false); stopSplitten() }}
            ...
            <div style={{ ...S.sheet, maxHeight: "86vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
              ...
              {nieuwKeuzeLijst && (
                <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 7 }}>
                  {zichtbaar.map(({ r, i }) => {
                    const inhoud = drinksOf(r).sort((a, b) => b.n - a.n).map(({ d, n }) => `${n}× ${d.name}`).join(", ")
                    const bezigHier = herhaalBezig && herhaalBron === i
                    const exact = () => {
                      if (herhaalBezig) return
                      const t: Record<string, number> = {}
                      drinksOf(r).forEach(({ d, n }) => { t[d.id] = n })
                      setHerhaalBron(i)
                      void (async () => { await bestelOpnieuw(t, i); sluit() })()
                    }
                    return (
                      <div key={r.id}
                        style={{ background: "#fff", border: "1.5px solid rgba(29,41,66,0.22)", borderRadius: 12, padding: "10px 12px" }}>
                        <span style={{ display: "block", minWidth: 0 }}>
                          <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
                            <span style={{ fontSize: 16, fontWeight: 800, color: "#1d2942" }}>{L.roundWord} {i + 1}</span>
                            {i === rounds.length - 1 && (
                              <span style={{ fontSize: 12, fontWeight: 800, borderRadius: 12, padding: "2px 8px", color: "#fff", background: "#1f8a4c" }}>{L.lastRoundPill}</span>
                            )}
                          </span>
                          <span style={{ display: "block", fontSize: 13.5, color: "#6b7484", fontWeight: 600, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{inhoud}</span>
                        </span>
                        {/* Twee wegen: exact hetzelfde meteen bestellen, of eerst aanpassen. */}
                        <div style={{ display: "flex", gap: 7, marginTop: 8 }}>
                          <button onClick={exact} disabled={herhaalBezig}
                            style={{ flex: 1.3, cursor: herhaalBezig ? "default" : "pointer", fontFamily: "inherit", border: "none", borderRadius: 10, padding: "9px 6px",
                              background: RAND, color: RANDTEKST, fontSize: 14.5, fontWeight: 800, opacity: herhaalBezig && !bezigHier ? 0.5 : 1 }}>
                            {bezigHier ? "…" : L.repeatOrderBtn}
                          </button>
                          <button onClick={() => { if (!herhaalBezig) { sluit(); openHerhaal(i) } }} disabled={herhaalBezig}
                            style={{ flex: 1, cursor: herhaalBezig ? "default" : "pointer", fontFamily: "inherit", borderRadius: 10, padding: "9px 6px",
                              background: "#fff", border: "1.5px solid rgba(224,138,0,0.65)", color: "#8a5e0f", fontSize: 14.5, fontWeight: 800, opacity: herhaalBezig ? 0.5 : 1 }}>
                            ✏️ {L.adjustOrderShort}
                          </button>
                        </div>
                      </div>
                ...
                {lijst.map(({ d, n }, i) => (
                  <div key={d.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: i < lijst.length - 1 ? "1px solid rgba(29,41,66,0.1)" : "none" }}>
                    ...
                    <span style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ minWidth: 34, textAlign: "center", fontSize: 21.5, fontWeight: 800, color: n > 0 ? "#c98a00" : "#a7b0bf" }}>{n}×</span>
      ...
      {kiesWeg && (() => {
        // Meer dan één gast heeft dit drankje staan, dus vragen we bij wie het weg mag in
        // plaats van er zelf een aan te wijzen. Alleen de aftrek wordt onthouden; opslaan
          ...
          <div style={{ ...S.overlay, zIndex: 72 }} onClick={() => setKiesWeg(null)}>
            <div style={S.sheet} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                {k.opties.map((o) => (
                  <button key={o.id} style={{ ...S.btn, fontSize: 17, padding: "12px 14px", textAlign: "left" }}
                    onClick={() => {
                      setEditDraft((c) => c ? {
                        ...c,
                        drinks: { ...c.drinks, [k.did]: Math.max(0, (c.drinks[k.did] ?? 0) - 1) },
                        weg: { ...c.weg, [k.did]: [...(c.weg[k.did] ?? []), o.id] },
                      } : c)
                      setKiesWeg(null)
                    }}>{o.naam}{o.n > 1 ? ` (${o.n}\u00d7)` : ""}</button>
                ))}
              </div>
              <button style={{ background: "none", border: "none", width: "100%", fontSize: 16, color: "#6b7484", fontWeight: 700, cursor: "pointer" }}
                onClick={() => setKiesWeg(null)}>{L.cancel}</button>
            </div>
          </div>
        )
      })()}
      ...
      {/* De afrekening bij de gast: een eigen pagina met een terugpijl, geen tabblad.
          Dezelfde settlement-berekening als bij de beheerder, alleen zonder de knoppen
          die iets veranderen. */}
      {guestSettlePage && meId && (() => {
        const zonder = rondjesZonderBedrag()
        const mijnLeeg = rounds.find((r) => (r.amount || 0) <= 0.005 && drinks.some((d) => (r.orders[d.id]?.[meId] ?? 0) > 0) && r.startedBy === meId)
        const eersteLeeg = rounds.find((r) => (r.amount || 0) <= 0.005 && r.id !== openRoundId && r.status !== "open")
        const pre = zonder.length > 0 ? "\u00b1 " : ""
        const mijnGroep = settleGroups.find((g) => g.leden.some((x) => x.id === meId))
        const mijnTx = settlement.tx.filter((t) => t.from === mijnGroep?.label || t.to === mijnGroep?.label)
        const ikKrijg = mijnTx.filter((t) => t.to === mijnGroep?.label)
        const ikBetaal = mijnTx.filter((t) => t.to !== mijnGroep?.label)
        return (
          <div style={{ position: "absolute", inset: 0, zIndex: 40, background: MODUS_FAIR.bladzij, minHeight: "100dvh" }}>
            <div style={{ maxWidth: 560, margin: "0 auto", padding: "calc(env(safe-area-inset-top, 0px) + 14px) 16px 40px" }}>
              ...
              {/* Je eigen bedrag bovenaan: op een lijst van zes is je eigen regel zoeken
                  net te veel werk voor de vraag die je stelt. */}
              <div style={{ ...S.card, textAlign: "center", padding: 14 }}>
                {ikBetaal.length > 0 ? (<>
                  <div style={{ fontSize: 14, color: "#6b7484" }}>{L.youPayStill}</div>
                  <div style={{ fontSize: 27, fontWeight: 800, color: MODUS_FAIR.tekst, margin: "2px 0" }}>{pre}{show(ikBetaal.reduce((a, t) => a + t.amount, 0))}</div>
                  <div style={{ fontSize: 14, color: "#b35309", fontWeight: 800 }}>{ikBetaal.map((t) => t.to).join(" · ")}</div>
                </>) : ikKrijg.length > 0 ? (<>
                  <div style={{ fontSize: 14, color: "#6b7484" }}>{L.youGetStill}</div>
                  <div style={{ fontSize: 27, fontWeight: 800, color: "#1f8a4c", margin: "2px 0" }}>{pre}{show(ikKrijg.reduce((a, t) => a + t.amount, 0))}</div>
                  <div style={{ fontSize: 14, color: "#1f6b3a", fontWeight: 800 }}>{ikKrijg.map((t) => t.from).join(" · ")}</div>
                </>) : (
                  <div style={{ fontSize: 16, color: "#6b7484", fontWeight: 800 }}>{L.allSquareGuest}</div>
                )}
              </div>

              {zonder.length > 0 && (
                <div style={{ background: "rgba(240,165,0,0.14)", border: "1.5px solid rgba(200,138,0,0.6)", borderRadius: 12, padding: "11px 12px", marginBottom: 13 }}>
                  <div style={{ fontSize: 15.5, fontWeight: 800, color: "#6b4a00", marginBottom: 3 }}>{L.provisionalTitle}</div>
                  <div style={{ fontSize: 14, color: "#6b7484", lineHeight: 1.5 }}>
                    {mijnLeeg ? L.provisionalGuestMine(mijnLeeg.seq) : L.provisionalGuest(eersteLeeg?.seq ?? zonder[0])}
                  </div>
                  {/* Alleen een knop als jij het zelf kan oplossen. */}
                  {mijnLeeg && (
                    <button onClick={() => { setGuestSettlePage(false); setGuestTab("me"); setOpenRounds(new Set([mijnLeeg.id])); setGastBedrag({ id: mijnLeeg.id, tekst: "", pot: false }) }}
                      style={{ width: "100%", marginTop: 9, cursor: "pointer", border: "none", borderRadius: 9, padding: 10, fontSize: 14, fontWeight: 800, color: "#fff", background: "#a8720a", fontFamily: "inherit" }}>
                      {L.fillWord}
                    </button>
                  )}
                </div>
              ...
              <div style={{ ...S.card, padding: 12 }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-end", gap: 8, paddingBottom: 6, borderBottom: "1.5px solid rgba(29,41,66,0.2)", fontSize: 12.5, fontWeight: 800, letterSpacing: "0.04em" }}>
                  ...
                  <span style={{ width: 64, textAlign: "right", color: "#8b93a3", flexShrink: 0 }}>{L.equalColHead.toUpperCase()}</span>
                </div>
                {people.map((p) => {
                  const g = settleGroups.find((gg) => gg.leden.some((x) => x.id === p.id))
                  const tx = settlement.tx.filter((t) => t.from === g?.label || t.to === g?.label)
                  const kr = tx.filter((t) => t.to === g?.label)
                  const bt = tx.filter((t) => t.to !== g?.label)
                  const ik = p.id === meId
                  return (
                    <div key={p.id} style={{ padding: "8px 0", borderBottom: "1px solid rgba(29,41,66,0.06)", background: ik ? "rgba(13,124,140,0.05)" : undefined, marginLeft: ik ? -12 : 0, marginRight: ik ? -12 : 0, paddingLeft: ik ? 12 : 0, paddingRight: ik ? 12 : 0 }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                        <span style={{ flex: 1, minWidth: 0, fontSize: 15.5, fontWeight: ik ? 800 : 700 }}>
                          {p.name}{ik ? ` \u00b7 ${L.youWord}` : ""}
                          {p.left && <span style={{ fontSize: 12.5, color: "#6b7484", fontWeight: 700 }}> · {L.leftBadge}</span>}
                        </span>
                        <span style={{ width: 70, textAlign: "right", fontSize: 15, fontWeight: 800, color: "#1f8a4c", flexShrink: 0 }}>{pre}{show(consumption(p.id))}</span>
                        <span style={{ width: 64, textAlign: "right", fontSize: 14, fontWeight: 700, color: "#8b93a3", flexShrink: 0 }}>{show(equalShare)}</span>
                      </div>
                      {kr.length > 0 && <div style={{ fontSize: 13.5, color: "#1f6b3a", fontWeight: 700, marginTop: 2 }}>{kr.map((t) => L.getsFromGuest(pre + show(t.amount), t.from)).join(" · ")}</div>}
                      {bt.length > 0 && <div style={{ fontSize: 13.5, color: "#b35309", fontWeight: 700, marginTop: 2 }}>{bt.map((t) => L.paysToGuest(pre + show(t.amount), t.to)).join(" · ")}</div>}
                      {kr.length === 0 && bt.length === 0 && <div style={{ fontSize: 13.5, color: "#9aa3b2", marginTop: 2 }}>{L.allSquareGuest}</div>}
                    </div>
                  )
                })}
                <div style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "9px 0 2px", borderTop: "2px solid rgba(29,41,66,0.25)", marginTop: 2 }}>
                  <span style={{ flex: 1, minWidth: 0, fontSize: 16.5, fontWeight: 800 }}>{L.togetherDrank}</span>
                  <span style={{ width: 70, textAlign: "right", fontSize: 16, fontWeight: 800, color: "#1f8a4c", flexShrink: 0 }}>{show(grandTotal)}</span>
      ...
      {handoverTo && meId === handoverTo && !isAdmin && (
        <div style={{ ...S.overlay, zIndex: 75 }}>
          <div style={S.sheet}>
            <div style={{ fontSize: 19.5, fontWeight: 800, color: "#1d2942", marginBottom: 4 }}>
              {L.handoverOfferTitle(people.find((p) => p.claimedBy === ownerDevice)?.name || "?")}
            </div>
            <div style={{ fontSize: 15, color: "#4a5560", lineHeight: 1.5, marginBottom: 14 }}>{L.handoverOfferBody}</div>
            <button style={{ ...S.btnP, marginBottom: 9 }} onClick={acceptHandover}>{L.handoverAccept}</button>
            <button style={{ width: "100%", background: "none", border: "none", padding: "6px 0", cursor: "pointer", fontFamily: "inherit", fontSize: 14.5, fontWeight: 700, color: "#6b7484" }}
              onClick={declineHandover}>{L.handoverDecline}</button>
          </div>
        </div>
      )}
      {confirmDlg && (
        <div style={{ ...S.overlay, zIndex: 70 }} onClick={() => setConfirmDlg(null)}>
          <div style={S.sheet} onClick={(e) => e.stopPropagation()}>
            ...
            {/* dangerDirect: de actie waarvoor je het venster opende is de knop, en
                terugkeren is een link. Voor gevallen waar je al beslist hebt vóór je
                klikte — een rondje annuleren dat je net startte. De gewone "danger"
                doet het omgekeerd en blijft ongewijzigd voor de andere schermen. */}
            {confirmDlg.variant === "dangerDirect" ? (
              <>
      ...
      {betaalVraag && (() => {
        ...
        <div style={{ ...S.overlay, zIndex: 81 }} onClick={() => setBetaalVraag(false)}>
          <div style={S.sheet} onClick={(e) => e.stopPropagation()}>
            ...
            {/* Te snel op "Klaar" getikt en de bestelling nog niet kunnen doorgeven? Dan
                wil je het lijstje terug, niet de betaalvraag. */}
            <div style={{ display: "flex", gap: 8, marginTop: 2, paddingTop: 11, borderTop: "1px solid rgba(29,41,66,0.12)" }}>
              <button onClick={() => { setBetaalVraag(false); setBarNaAfronden(true); setBarFull(true) }}
                style={{ ...S.btn, flex: 1, fontSize: 14, padding: "11px 6px", color: MODUS_FAIR.tekst, borderColor: MODUS_FAIR.lijnZacht }}>{L.payBackToList}</button>
              <button onClick={() => setBetaalVraag(false)}
                style={{ ...S.btn, flex: 1, fontSize: 14, padding: "11px 6px", color: "#6b7484" }}>{L.paySkip}</button>
            </div>
          </div>
        </div>
        )
...
<span style={{ display: "inline-flex", verticalAlign: "-3px", marginRight: 3 }}><GsmIcoon size={16} kleur={MODUS_FAIR.tekst} /></span> <b>{L.theyTap}</b> {L.theyTapRest}<br />
              ...
              {/* De verrekening is geruststelling, geen instructie: eigen balkje, zodat
                  ze niet meeleest als iets wat jij nu moet doen. */}
              <div style={{ fontSize: 14, fontWeight: 700, color: MODUS_FAIR.tekst, background: MODUS_FAIR.tint, borderRadius: 10, padding: "8px 10px", marginTop: 10 }}>{L.settledAfter}</div>
            </div>
            {herhaalbaar && (
              <div style={{ background: MODUS_FAIR.vlak, border: `1px dashed ${MODUS_FAIR.randZacht}`, borderRadius: 11, padding: "10px 12px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 9 }}>
                ...
                <button onClick={() => { setStartCheck(false); repeatRound(); void startAsRunner(); naarHalerKop() }}
            ...
            <div style={{ display: "flex", gap: 9 }}>
              ...
              <button onClick={() => { setStartCheck(false); void startAsRunner(); naarHalerKop() }}
  ...
  const renderVoorWieStrook = (inRaster = false) => (
    (() => {
      ...
      // In de samen-stand zei de strook alleen "Je tikt aan voor iedereen". Dat is in Neem
      // zelf op altijd zo, dus hij valt weg; per persoon blijft hij wél staan.
      if (!perPersoon) return null
  ...
  if (groupId && !isAdmin && meId) {
    ...
    // Er loopt een rondje dat iemand ánders haalt: dan zweeft de kop mee en komen de
    ...
    // De zwevende onderbalk is er voor iedereen die nog een eigen keuze moet bevestigen,
    // dus ook voor wie het rondje startte. Alleen de tekst erboven verschilt: die gaat
    // bij de starter over zijn eigen keuze, niet over doorgeven aan iemand anders.
    const ikStartte = settle && !!openRoundId && startedBy === meId
    const zweefRondje = settle && !!openRoundId && !!startedBy && startedBy !== meId
    const zweefOnder = settle && !!openRoundId && !!startedBy
    ...
    // Tot drie soorten voluit; daarboven volstaat het aantal, anders wordt de regel lang.
    const mijnKort = mijn.length > 0 && mijn.length <= 3
      ? mijn.map((d) => `${aQty(d.id, meId)}× ${d.name}`).join(" · ")
      : L.youAreDone(mijnAantal)
    ...
    return (
      <div style={S.page}><div style={{ ...S.wrap }}>
        ...
        {(guestTab === "group" || !(orderingOpen || rounds.length > 0)) && (
          <div style={S.card}>
            <div style={{ marginBottom: 12 }}>
              ...
              {/* Dezelfde potkaart als de beheerder onder zijn groepsnaam heeft, maar dood:
                  kijken en uitklappen mag, inleggen en bijstellen blijft bij wie de groep
                  beheert. Zonder pot verschijnt er niets. */}
              {potContribTotal > 0.005 && (
                <div style={{ background: MODUS_FAIR.vlak, border: `1px solid ${MODUS_FAIR.lijnZacht}`, borderRadius: 12, padding: "10px 12px", marginTop: 11 }}>
                  <div style={{ ...S.row, justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                      <ZakjeIcoon size={16} />
                      <span style={{ fontSize: 16, fontWeight: 800, color: MODUS_FAIR.rand }}>{potIsCard ? L.drinkCard : L.potTitle}</span>
                    </span>
                    <span style={{ fontSize: 16, fontWeight: 800, color: "#1d2942" }}>{euro(potContribTotal)}</span>
                  </div>
                  <div onClick={() => setPotDetails((v) => !v)} style={{ ...S.row, justifyContent: "space-between", alignItems: "center", gap: 8, marginTop: 5, cursor: "pointer" }}>
                    <span style={{ fontSize: 12.5, color: MODUS_FAIR.label, minWidth: 0 }}>{L.potSummary(people.filter((pp) => contribOf(pp.id) > 0.005).length, euro(Math.max(0, potRemaining)))}</span>
                    <span style={{ flexShrink: 0, fontSize: 13, fontWeight: 700, color: MODUS_FAIR.rand }}>{L.detailsWord} {potDetails ? "▴" : "▾"}</span>
                  </div>
                  {potDetails && (
                    <div style={{ marginTop: 9, paddingTop: 9, borderTop: `1px solid ${MODUS_FAIR.lijnZacht}`, display: "flex", flexDirection: "column", gap: 5 }}>
                      {potRounds.map((r, i) => {
                        const tot = Object.values(r.amounts).reduce((a, b) => a + (b || 0), 0)
                        const wie = people.filter((pp) => (r.amounts[pp.id] || 0) > 0)
                        return (
                          <div key={r.id} style={{ fontSize: 13, color: "#1d2942", lineHeight: 1.45 }}>
                            <b style={{ fontWeight: 800 }}>{L.nthDeposit(i + 1)} · {euro(tot)}</b>
                            {wie.length > 0 && <span style={{ color: MODUS_FAIR.label }}> — {wie.map((pp) => `${pp.name}${pp.id === meId ? ` (${L.youWord})` : ""} ${euro(r.amounts[pp.id] || 0)}`).join(" · ")}</span>}
                          </div>
                        )
                      })}
                      <div style={{ ...S.row, justifyContent: "space-between", marginTop: 3 }}>
                        <span style={{ fontSize: 13, color: MODUS_FAIR.label }}>{L.alreadySpent} <b style={{ color: "#c88a1a" }}>{euro(potSpent)}</b></span>
                        <span style={{ fontSize: 14, fontWeight: 800, color: MODUS_FAIR.rand }}>{L.stillLeft(euro(Math.max(0, potRemaining)))}</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {people.map((p) => {
                ...
                return (
                  <div key={p.id} style={{ ...S.row, justifyContent: "space-between", padding: "8px 11px", borderRadius: 10,
                    background: p.left ? "rgba(29,41,66,0.05)" : benIkHet ? "rgba(31,138,76,0.08)" : VLAK1,
                    border: p.left ? "1px dashed rgba(29,41,66,0.22)" : benIkHet ? "1px solid rgba(31,138,76,0.3)" : "1px solid rgba(29,41,66,0.1)" }}>
                    <span style={{ fontSize: 17.5, fontWeight: benIkHet && !p.left ? 800 : 700, color: p.left ? "#6b7484" : p.named ? "#1d2942" : "#9aa3b2" }}>
                      {p.name}
                      {benIkHet && !p.left && <span style={{ fontSize: 14.5, color: "#1f6b3a", fontWeight: 800 }}> · {L.youMark}</span>}
                      {isHost && !benIkHet && <span style={{ fontSize: 14.5, color: "#8a5e0f", fontWeight: 800 }}> · {L.hostMark}</span>}
                    </span>
                    <span style={{ fontSize: 14.5, color: p.left ? "#6b7484" : aangemeld ? "#8a5e0f" : "#9aa3b2", fontWeight: 700 }}>
                      {p.left ? L.leftBadge : aangemeld ? L.scannedSelf : L.notScannedYet}
                    </span>
                  </div>
                )
              })}
          ...
          <>
            ...
            {rounds.length === 0 ? (
              ...
              <>
                {rounds.map((r) => {
                  ...
                  return (
                    <div key={r.id} style={{ ...S.card, padding: 11, marginBottom: 9 }}>
                      ...
                      {/* Wacht er iemand op dít bedrag om te kunnen vertrekken, dan staat dat
                          erbij — anders is "vul in" gewoon huiswerk dat kan wachten. */}
                      {ikHaalde && (r.amount || 0) <= 0.005 && !!r.leaveWaitFor && r.leaveWaitFor !== meId && (
                        <div style={{ background: "rgba(224,138,0,0.16)", borderRadius: 9, padding: "8px 9px", marginTop: 8, fontSize: 12, color: "#8a5e0f", lineHeight: 1.4 }}>
                          <b style={{ fontWeight: 800 }}>{L.adminLeaveBanner(people.find((p) => p.id === r.leaveWaitFor)?.name || "?", r.seq)}</b> {L.adminLeaveBannerBody}
                        </div>
                      )}
                ...
                {groepAf && (
                  <div style={{ background: "rgba(31,138,76,0.1)", border: "1.5px solid rgba(31,138,76,0.4)", borderRadius: 13, padding: 12, textAlign: "center", marginBottom: 10 }}>
                ...
                {/* Zelfde afweging als bij de beheerder: de tabbalk staat er al. Alleen
                    terwijl er een rondje loopt is deze knop een echte snelweg terug. */}
                {!!openRoundId && (
        ...
        <>
        {/* Loopt er een rondje dat iemand anders haalt, dan zweeft alles wat je tijdens
            het kiezen nodig hebt bovenaan mee: wie haalt, wie nog moet kiezen, en de
            categorieknoppen. Zonder dat laatste moet je terug omhoog scrollen om van
            Bier naar Fris te gaan. De knoppen om te bevestigen staan onderaan, binnen
          ...
          <div ref={gastRondjeKop} style={{ position: "sticky", top: 0, zIndex: 8, paddingTop: 2, marginBottom: 10, scrollMarginTop: 0, background: MODUS_FAIR.bladzij }}>
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: -4, marginBottom: 8 }}>
              {nogAanwezig.map((pp) => {
                ...
                const niets = openAnswers[pp.id] === "skip"
                return (
                  <span key={pp.id} style={{ fontSize: 12.5, fontWeight: 800, padding: "4px 9px", borderRadius: 999,
                    background: niets ? "rgba(29,41,66,0.08)" : ok ? "rgba(31,138,76,0.14)" : "rgba(240,165,0,0.16)",
                    color: niets ? "#6b7484" : ok ? "#1f6b3a" : "#8a5e0f" }}>
                    {niets ? "—" : ok ? "✓" : "⏳"} {pp.id === meId ? L.youWord : pp.name}
                  </span>
                )
              })}
            </div>
        ...
        {/* De zwevende onderbalk: hier bevestig je, of geef je aan dat je niets neemt.
            Zolang je niet bevestigde staat er een gouden regel boven — zonder die tik
            weet de haler niet dat je klaar bent, ook al tikte je al drankjes aan. */}
        {zweefOnder && (
          <div style={{ position: "sticky", bottom: 0, zIndex: 8, marginTop: 12, paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 4px)" }}>
            <div style={{ background: "#fff", border: `1.5px solid ${MODUS_FAIR.lijnZacht}`, borderRadius: 16, padding: 11, boxShadow: "0 -8px 20px -10px rgba(13,124,140,0.7)" }}>
              {ikBenKlaar ? (
                <div style={{ display: "flex", alignItems: "center", gap: 12, borderRadius: 13, padding: 14,
                  background: ikNamNiets ? "rgba(29,41,66,0.05)" : "rgba(31,138,76,0.1)",
                  border: `1.5px solid ${ikNamNiets ? "rgba(29,41,66,0.16)" : "rgba(31,138,76,0.3)"}` }}>
                  <span style={{ flexShrink: 0, width: 36, height: 36, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19, fontWeight: 800, color: "#fff", background: ikNamNiets ? "#8b93a3" : "#1f8a4c" }}>{ikNamNiets ? "—" : "✓"}</span>
                  <span style={{ minWidth: 0, display: "flex", flexDirection: "column" }}>
                    <b style={{ fontSize: 16.5, color: "#1d2942" }}>{ikNamNiets ? L.youTakeNothing : ikStartte ? L.yourChoiceFixed : L.passedOnTo(runnerName())}</b>
                    <span style={{ fontSize: 13.5, fontWeight: 700, color: ikNamNiets ? "#6b7484" : "#1f6b3a", marginTop: 2 }}>{ikNamNiets ? (ikStartte ? L.roundWord + " " + roundNr : runnerName()) : mijnKort}</span>
                  </span>
                  <button onClick={() => antwoordRondje("different")}
                    style={{ marginLeft: "auto", flexShrink: 0, background: "#fff", border: "1px solid rgba(29,41,66,0.25)", color: "#1d2942", fontSize: 14, fontWeight: 800, padding: "10px 15px", borderRadius: 999, cursor: "pointer", fontFamily: "inherit" }}>{L.changeWord}</button>
                </div>
              ) : (
                <>
                  <div style={{ fontSize: 14, fontWeight: 800, color: "#8a5e0f", background: "rgba(240,165,0,0.14)", borderRadius: 10, padding: "9px 10px", marginBottom: 9, textAlign: "center", lineHeight: 1.35 }}>
                    ☝️ {ikStartte ? L.yourOwnNotConfirmed : herhaaldVoorMij.current ? L.sameAsLastHint : L.notPassedOn(runnerName())}
                  </div>
                  <div style={{ display: "flex", gap: 7 }}>
                    ...
                    <button onClick={() => antwoordRondje("same")} disabled={mijnAantal === 0}
                      style={{ flex: 1.3, background: MODUS_FAIR.rand, border: "none", color: "#fff", fontSize: 16.5, fontWeight: 800, padding: "15px 6px", borderRadius: 12, cursor: mijnAantal === 0 ? "default" : "pointer", opacity: mijnAantal === 0 ? 0.45 : 1, fontFamily: "inherit", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                      {L.confirmMine}
                      {mijnAantal > 0 && <span style={{ background: "#F5B301", color: "#2a2110", borderRadius: 999, minWidth: 23, height: 23, padding: "0 6px", fontSize: 13, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{mijnAantal}</span>}
                    </button>
                  </div>
                </>
              )}
  ...
  if (view === "start" && !herkomstGelezen && !groupId && !onboardedOnce) {
  ...
  if (view === "settings") {
    return (
      <div style={S.page} onClick={() => { setCoinInfo(false); setDepositInfo(false) }}><div style={S.wrap}>
        ...
        {showPot && renderPotModal()}
        {renderDialogs()}
        {/* Ook hier de drie tabbladen: dit ís het blad "Groep & QR". Zonder balk was de
            terugknop onderaan de enige uitweg, en die staat er nu alleen nog tijdens een
            lopend rondje. */}
        <AdminTabs />
              ...
              {people.map((p, idx) => {
                ...
                return (
                  <div key={p.id} style={{ position: "relative" }}>
                    ...
                    {/* Scande deze persoon zelf de QR, dan zit hij op zijn eigen gsm en
                        tikt hij zelf aan. Dat is iets anders dan een plaats die jij met de
                        hand bijzette: die naam moet jij onderhouden. Het toestelletje zegt
                        welke van de twee het is, zonder er woorden aan vuil te maken. */}
                    {!ikZelf && !!p.claimedBy && (
                      <span title={L.viaQrTitle} style={{ position: "absolute", right: 7, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", display: "inline-flex" }}>
                        <svg width="15" height="21" viewBox="0 0 15 21" aria-label={L.viaQrTitle} role="img">
                          <rect x="0.9" y="0.9" width="13.2" height="19.2" rx="2.6" fill="none" stroke={MODUS_FAIR.rand} strokeWidth="1.7" />
                          <line x1="5.4" y1="3.4" x2="9.6" y2="3.4" stroke={MODUS_FAIR.rand} strokeWidth="1.5" strokeLinecap="round" />
                          <circle cx="7.5" cy="17.1" r="1.25" fill={MODUS_FAIR.rand} />
                        </svg>
                      </span>
                    )}
                  </div>
        ...
        <div style={{ marginTop: 24 }}>
          {(() => {
            ...
            // Staat de tabbalk erboven, dan is "terug" alleen nog zinvol als je middenin
            // een rondje zit: dan is het een sprong terug naar waar je bezig was. Zonder
            // open rondje brengen de tabbladen je overal, en is de knop een derde weg
            // naar hetzelfde scherm.
            const terug = settingsBackTo === "quickSettle" ? { label: L.backToSettle, ga: () => setView("quickSettle") }
              : (settingsBackTo === "order" && (!tabsHier || !!openRoundId)) ? { label: L.backToRound(roundNr), ga: naarRondje }
              : null
            ...
            {isAdmin && settle && !fromQuick && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid rgba(29,41,66,0.12)" }}>
                {handoverTo ? (
                  <div style={{ background: "rgba(224,138,0,0.12)", border: "1px solid rgba(224,138,0,0.5)", borderRadius: 11, padding: "10px 12px", fontSize: 14, color: "#8a5e0f", lineHeight: 1.45 }}>
                    {L.handoverTo(people.find((p) => p.id === handoverTo)?.name || "?")} — ⏳
                    <button onClick={declineHandover} style={{ display: "block", marginTop: 7, background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: 13.5, fontWeight: 800, color: "#a8720a", textDecoration: "underline" }}>{L.cancel}</button>
                  </div>
                ) : !handoverOpen ? (
                  <button style={{ ...S.btn, width: "100%", fontSize: 16 }} onClick={() => setHandoverOpen(true)}>👑 {L.handoverTitle}</button>
                ) : (
                  <div style={{ background: "#fff", border: "1.5px solid rgba(29,41,66,0.18)", borderRadius: 12, padding: 12 }}>
                    <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 2 }}>{L.handoverTitle}</div>
                    <div style={{ fontSize: 13.5, color: "#6b7484", lineHeight: 1.45, marginBottom: 10 }}>{L.handoverSub}</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      {people.filter((p) => !p.left && p.claimedBy !== ownerDevice).map((p) => {
                        const kan = !!p.claimedBy
                        return (
                          <button key={p.id} disabled={!kan} onClick={() => kan && offerHandover(p.id)}
                            style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", cursor: kan ? "pointer" : "default", fontFamily: "inherit", textAlign: "left",
                              background: kan ? "#fff" : "#f7f7f7", border: kan ? "1px solid rgba(29,41,66,0.2)" : "1px dashed rgba(29,41,66,0.2)",
                              borderRadius: 9, padding: "9px 11px", fontSize: 14.5, fontWeight: kan ? 800 : 600, color: kan ? "#1d2942" : "#9aa3b2" }}>
                            <span>{p.name}</span>
                            {!kan && <span style={{ fontSize: 12.5, fontWeight: 700 }}>{L.handoverNotScanned}</span>}
                          </button>
                        )
                      })}
                    </div>
                    <button onClick={() => setHandoverOpen(false)} style={{ width: "100%", marginTop: 9, background: "none", border: "none", padding: "5px 0", cursor: "pointer", fontFamily: "inherit", fontSize: 13.5, fontWeight: 700, color: "#6b7484" }}>{L.cancel}</button>
                  </div>
                )}
              </div>
            )}
  ...
  if (view === "order") {
    ...
    return (
      <div style={S.page}><div style={S.wrap}>
                ...
                return (
                  <div key={d.id} onClick={() => { if (settle && !bezig) setGeenRondje(true) }}
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
                      <span style={{ flex: 1, minWidth: 0, fontSize: 17.5, fontWeight: tot > 0 ? 800 : 600, color: tot > 0 ? "#1f6b3a" : themaNaam ? "#2c3752" : "#4a5567", lineHeight: 1.25 }}>{d.emoji} {d.name}</span>
                      {/* Een drankje dat jij zelf toevoegde, pas je hier aan. Vroeger moest je
                          daarvoor eerst "Eigen drankje?" opnieuw openen en het in de lijst
                          onderaan opzoeken — terwijl je er net naar zat te kijken. */}
                      {d.custom && d.by === me.current && (
                        <span onClick={(e) => { e.stopPropagation(); setEditDrinkKey(d.id); setNdName(d.name); setNdPrice(String(d.price).replace(".", ",")) }}
                          title={L.editWord} aria-label={L.editWord}
                          style={{ flexShrink: 0, fontSize: 14, lineHeight: 1, padding: 3, marginTop: -1, opacity: 0.55, cursor: "pointer" }}>✏️</span>
                      )}
                    </div>
                    <div style={{ ...S.row, justifyContent: "space-between", marginTop: 7 }}>
        ...
        {settle && startedBy && meId ? (() => {
          /* Er loopt een rondje. De beheerder kiest dan gewoon zoals iedereen, met dezelfde
             zwevende balk als een gast — ook wanneer hij zélf haalt. Het afronden van het
             rondje staat in de kaart bovenaan, zodat de balk hier altijd over jouw eigen
             keuze gaat. Haalt iemand anders, dan komt de beheerderslink er nog onder. */
          const ikHaalZelf = startedBy === meId
          ...
          const mijneB = drinks.filter((d) => aQty(d.id, meId) > 0)
          const mijnKortB = mijneB.length > 0 && mijneB.length <= 3
            ? mijneB.map((d) => `${aQty(d.id, meId)}× ${d.name}`).join(" · ")
            : L.youAreDone(mijnN)
          return (
            ...
            <div style={{ background: "#fff", border: `1.5px solid ${MODUS_FAIR.lijnZacht}`, borderRadius: 16, padding: 11, boxShadow: "0 -8px 20px -10px rgba(13,124,140,0.7)" }}>
              {ikKlaar ? (
                <div style={{ display: "flex", alignItems: "center", gap: 12, borderRadius: 13, padding: 14,
                  ...
                  border: `1.5px solid ${ikNiets ? "rgba(29,41,66,0.16)" : "rgba(31,138,76,0.3)"}` }}>
                  <span style={{ flexShrink: 0, width: 36, height: 36, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19, fontWeight: 800, color: "#fff", background: ikNiets ? "#8b93a3" : "#1f8a4c" }}>{ikNiets ? "—" : "✓"}</span>
                  <span style={{ minWidth: 0, display: "flex", flexDirection: "column" }}>
                    <b style={{ fontSize: 16.5, color: "#1d2942" }}>{ikNiets ? L.youTakeNothing : ikHaalZelf ? L.yourChoiceFixed : L.passedOnTo(runnerName())}</b>
                    <span style={{ fontSize: 13.5, fontWeight: 700, color: ikNiets ? "#6b7484" : "#1f6b3a", marginTop: 2 }}>{ikNiets ? (ikHaalZelf ? L.roundWord + " " + roundNr : runnerName()) : mijnKortB}</span>
                  </span>
                  <button onClick={() => antwoordRondje("different")}
                    style={{ marginLeft: "auto", flexShrink: 0, background: "#fff", border: "1px solid rgba(29,41,66,0.25)", color: "#1d2942", fontSize: 14, fontWeight: 800, padding: "10px 15px", borderRadius: 999, cursor: "pointer", fontFamily: "inherit" }}>{L.changeWord}</button>
                </div>
                ...
                <>
                  <div style={{ fontSize: 14, fontWeight: 800, color: "#8a5e0f", background: "rgba(240,165,0,0.14)", borderRadius: 10, padding: "9px 10px", marginBottom: 9, textAlign: "center", lineHeight: 1.35 }}>
                    ☝️ {ikHaalZelf ? L.yourOwnNotConfirmed : L.notPassedOn(runnerName())}
                  </div>
                  <div style={{ display: "flex", gap: 7 }}>
                    <button onClick={() => antwoordRondje("skip")}
                      style={{ flex: 1, background: "#fff", border: "1.5px solid rgba(29,41,66,0.28)", color: "#6b7484", fontSize: 15, fontWeight: 800, padding: "15px 6px", borderRadius: 12, cursor: "pointer", fontFamily: "inherit" }}>{L.nothingForMeBtn}</button>
      ...
      <div style={S.page}><div style={S.wrap}>
        ...
        <div style={S.card}>
          ...
          <div style={{ ...S.row, justifyContent: "space-between", gap: 8, borderTop: "1px dashed rgba(29,41,66,0.25)", marginTop: 8, paddingTop: 8 }}>
            {/* "Iemand mag gaan halen" hoort bij het moment vóór de toog. In QR-modus
                sta je hier ná het barlijstje: de drankjes zijn al gehaald. */}
        ...
        <div style={S.card}>
          ...
          <>
                  {/* Twee vlakken in plaats van pillen: één ervan staat altijd aan, dus
                      het is een keuze en geen aanzetknop. De namenrij is weg — wie het
                      rondje haalde is de betaler, dat hoeft niemand nog aan te wijzen.
                      Alleen bij "gewoon rondjes" mag je een ander aanwijzen. */}
                  <div style={{ marginBottom: 4 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 800, color: "#6b7484", letterSpacing: "0.04em", marginBottom: 7 }}>{L.paidWithHead}</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => { setPayPot(false); const wie = startedBy ? [startedBy] : (meId ? [meId] : []); setPayPersons(wie); autoSplit(wie, false) }}
                        style={{ flex: 1, cursor: "pointer", fontFamily: "inherit", borderRadius: 12, padding: "13px 6px", textAlign: "center",
                          background: !payPot ? MODUS_FAIR.tekst : "#fff", border: `2px solid ${!payPot ? MODUS_FAIR.tekst : "rgba(29,41,66,0.2)"}` }}>
                        <span style={{ display: "block", fontSize: 21, marginBottom: 3 }}>💸</span>
                        <span style={{ display: "block", fontSize: 16, fontWeight: 800, color: !payPot ? "#fff" : "#4a5560" }}>{L.paidSelfBtn}</span>
                      </button>
                      {potChosen && st.potAvail > 0.005 && (
                        <button onClick={() => { setPayPot(true); setPayPersons([]); autoSplit([], true) }}
                          style={{ flex: 1, cursor: "pointer", fontFamily: "inherit", borderRadius: 12, padding: "13px 6px", textAlign: "center",
                            background: payPot ? "#eef4fb" : "#fff", border: `2px solid ${payPot ? "#2f6fb5" : "rgba(47,111,181,0.3)"}` }}>
                          <span style={{ display: "block", marginBottom: 3, lineHeight: 1 }}>{potIsCard ? <span style={{ fontSize: 21 }}>💳</span> : <ZakjeIcoon size={21} />}</span>
                          <span style={{ display: "block", fontSize: 16, fontWeight: 800, color: "#2f5693" }}>{potIsCard ? L.drinkCard : L.paidPotBtn}</span>
                          <span style={{ display: "block", fontSize: 12, color: "#8b93a3", marginTop: 1 }}>{L.potLeftShort(euro(st.potAvail))}</span>
                        </button>
                      )}
                    </div>
                    <div style={{ fontSize: 13, color: "#6b7484", lineHeight: 1.5, padding: "9px 2px 0" }}>
                      {payPot ? L.paidPotHint : L.paidSelfHint}
                    </div>
                    {/* Pot te klein? Geen keuzewerk: de app deelt het en zegt wat er gebeurt. */}
                    {payPot && st.total - st.potAvail > 0.005 && (
                      <div style={{ background: "rgba(240,165,0,0.12)", border: "1px solid rgba(224,138,0,0.45)", borderRadius: 10, padding: "9px 11px", marginTop: 9, fontSize: 13.5, color: "#8a5e0f", lineHeight: 1.5 }}>
                        {L.potShortNote(euro(st.potAvail), euro(st.total - st.potAvail))}
                      </div>
                    )}
                  </div>
        ...
        {/* Annuleren en bestelling wijzigen horen niet meer op dit scherm: de drankjes
            staan al op de toog. In QR-modus verdwijnen ze; bij zelf noteren blijven ze,
            want daar is er nog geen bestelling gedaan. */}
        {!(settle && !fromQuick) && (
    ...
    return (
      <div style={S.page}><div style={S.wrap}>
        ...
        {isAdmin && rounds.filter((r) => r.leaveWaitFor).map((r) => {
          const naam = people.find((p) => p.id === r.leaveWaitFor)?.name || "?"
          return (
            <div key={r.id} style={{ background: "rgba(224,138,0,0.14)", border: "1.5px solid rgba(224,138,0,0.6)", borderRadius: 12, padding: "10px 12px", marginBottom: 10 }}>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#8a5e0f" }}>{L.adminLeaveBanner(naam, r.seq)}</div>
              <div style={{ fontSize: 13, color: "#6b4a00", lineHeight: 1.4, margin: "3px 0 9px" }}>{L.adminLeaveBannerBody}</div>
              <button onClick={() => { const idx = rounds.indexOf(r); setAllRoundsOpen(false); setOpenRound(idx); startEditRound(r) }}
                style={{ width: "100%", cursor: "pointer", border: "none", borderRadius: 9, padding: 10, fontSize: 13.5, fontWeight: 800, color: "#fff", background: "#a8720a", fontFamily: "inherit" }}>
                {L.fillAmountForLeaveBtn(r.seq)}
              </button>
            </div>
          )
        })}
        ...
        {groepAf && (
          <div style={{ background: "rgba(31,138,76,0.1)", border: "1.5px solid rgba(31,138,76,0.4)", borderRadius: 13, padding: 12, textAlign: "center", marginBottom: 11 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: "#1f6b3a" }}>🍻 {L.nightClosedTitle}</div>
            <div style={{ fontSize: 14, color: "#4a5560", lineHeight: 1.45, margin: "3px 0 10px" }}>{L.nightClosedBody}</div>
            <button onClick={goFinal} style={{ ...S.btnP, padding: "11px 14px", fontSize: 16 }}>{L.seeFinalBtn}</button>
            {/* Wijzigen na het afsluiten is een bewuste stap: anders schuift de
                "definitieve" afrekening onder ieders neus op. */}
            <button onClick={() => setConfirmDlg({ msg: L.reopenNight, yes: L.reopenNight, no: L.cancel,
                onYes: () => { setConfirmDlg(null); setGroepAf(false); if (groupId) supabase.from("party_groups").update({ finalized: false }).eq("id", groupId) } })}
              style={{ width: "100%", marginTop: 8, background: "none", border: "none", padding: "5px 0", cursor: "pointer", fontFamily: "inherit", fontSize: 13.5, fontWeight: 800, color: "#6b7484", textDecoration: "underline" }}>
              {L.reopenNight}
            </button>
          </div>
        )}
        {rounds.map((r, idx) => ({ r, idx })).map(({ r, idx }) => {
          ...
          return (
            <div key={idx} style={{ ...S.card, padding: 0, overflow: "hidden" }}>
              <div style={{ cursor: "pointer", padding: 14 }} onClick={() => { if (allRoundsOpen) { setAllRoundsOpen(false); setOpenRound(idx) } else { setOpenRound(open ? null : idx) }  }}>
        ...
        {!fromQuick && paidCount > 0 && laatsteRondjeKlaar() && !((settle || opNaam) && unassignedAllRounds > 0) && <>
          {/* "Nieuw rondje" stond hier als hoofdactie, maar de tabbalk brengt je met één
              tik naar Drankjes — dit scherm gaat over wat al gebeurde. Een half afgemaakt
              rondje is iets anders: dat afronden blijft wel bereikbaar. */}
          <div style={{ display: "flex", gap: 10 }}>
            <button style={{ ...(unfinishedRound ? S.btn : S.btnP), flex: 1, ...(unfinishedRound ? {} : { width: "100%" }) }} onClick={goFinal}>{L.settleBtn}</button>
            {unfinishedRound && <button style={{ ...S.btnP, flex: 2 }} onClick={resumeRound}>{L.continueRound(roundNr)}</button>}
          </div>
  ...
  if (view === "fairIntro") {
    // Wat er gaat gebeuren, vóór je eraan begint. Zonder dit scherm begon het splitten
    // met een vraag over het aantal personen, zonder dat je wist hoeveel er nog volgden
    // of dat je tussendoor mocht stoppen. Dat laatste staat er met opzet bij: het haalt
    // de drempel weg om er gewoon aan te beginnen.
    const stappen: [string, string][] = [
      [L.fairIntroStep1, L.fairIntroStep1Sub],
      [L.fairIntroStep2, L.fairIntroStep2Sub],
      [L.fairIntroStep3, L.fairIntroStep3Sub],
    ]
    return (
      <div style={S.page}><div style={S.wrap}>
        <Header kaal />
  ...
  if (view === "roundsOverview") {
    ...
    return (
      <div style={S.page}><div style={S.wrap}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10, padding: "0 4px 9px", marginBottom: 0, borderBottom: "1px solid rgba(29,41,66,0.18)" }}>
          ...
          {/* Zonder één bedrag zei "€0,00" hier niets, behalve dat er iets leek te ontbreken. */}
          {metBedrag > 0 && <span style={{ fontSize: 24, fontWeight: 800, color: "#c98a00" }}>{euro(totalCost)}</span>}
        </div>

        {/* De regel "Eerlijk verdelen? — Nu aanvullen" stond hier; elk rondje toont in
            zijn eigen amberen kader al wat er nog moet gebeuren. */}
          ...
          {rounds.map((r) => {
            ...
            return (
              <div key={r.id} style={{ ...S.card, padding: 0, overflow: "hidden",
                ...(editRoundId === r.id ? { boxShadow: "inset 0 0 0 2px rgba(240,165,0,0.55)", background: "#fffdf3" } : invulRij ? { border: "2px solid rgba(224,104,92,0.65)", background: "rgba(224,104,92,0.05)" } : settle && geenBedrag ? { border: "1.5px dashed rgba(224,138,0,0.7)", background: "#fffaf0" } : {}) }}>
                <div onClick={() => toggle(r.id)} style={{ padding: "12px 14px", cursor: "pointer", background: editRoundId === r.id ? "rgba(240,165,0,0.1)" : open ? "rgba(240,165,0,0.06)" : invulRij ? "rgba(224,104,92,0.07)" : "#fff" }}>
                  <div style={{ ...S.row, justifyContent: "space-between", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6, minWidth: 0 }}>
                      ...
                      {editRoundId !== r.id && (() => {
                        ...
                        return (<>
                          {nogToe > 0 && pil(L.noNamePill(nogToe), "#b0402f", "rgba(224,104,92,0.14)")}
                          {geenBedrag && pil(L.noAmountPill, "#8a5e0f", "rgba(240,165,0,0.16)")}
                        </>)
                      })()}
                    </div>
                    <div style={{ ...S.row, gap: 9, flexShrink: 0 }}>
                      ...
                      {editRoundId === r.id ? null : (
                        <>
                          ...
                          {/* Het woord zegt wat er gebeurt, de pijl welke kant op. Klein en
                              grijs, want bij tien rondjes onder elkaar mag dit niet roepen.
                              Een zwevende pil onder elk rondje deed dat wel, en overlapte
                              bovendien de regel eronder. */}
                          {settle ? (
                            ...
                            <span style={{ flexShrink: 0, fontSize: 14.5, fontWeight: 800, color: open ? "#8a5e0f" : "#6b7484", whiteSpace: "nowrap" }}>
                              {L.detailsWord} {open ? "▴" : "▾"}
                            </span>
                          )}
                        </>
                  ...
                  {/* Hier stond de lijst met drankjes over twee regels. Bij een rondje met
                      negen drankjes werd dat een grijze muur; het aantal zegt genoeg, en
                      wát erin zat lees je achter "Toon details". */}
                  <div style={{ fontSize: 15.5, color: "#6b7484", fontWeight: 600, marginTop: 4 }}>
                    {L.drinksCount(items)}
                    {(r.amount || 0) > 0.005 && <> · {L.paidNote(euro(r.amount))}</>}
                    {(r.amount || 0) > 0.005 && ((r.potPart || 0) > 0.005
                ...
                {/* Zelfde vorm als de "+ n meer"-pil bij de drankjes: zwevend over de
                    onderrand, omlijnd in plaats van gevuld. Eén manier om in de hele app
                    te zeggen dat er meer achter zit. */}
                ...
                {open && (() => {
                  ...
                  const rijen = bewerk && dr
                    ? [...basisRijen, ...drinks.filter((d) => (dr.drinks[d.id] ?? 0) > 0 && !basisRijen.some((x) => x.d.id === d.id)).map((d) => ({ d, n: 0 }))]
                    : basisRijen
                  const resterend = bewerk && dr ? drinks.filter((d) => !rijen.some((x) => x.d.id === d.id)) : []
                  return (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      {rijen.map(({ d, n }) => {
                        ...
                        <div key={d.id} style={{ ...S.row, justifyContent: "space-between", padding: "3px 0" }}>
                          <span style={{ fontSize: 17.5, fontWeight: 700, minWidth: 0 }}>{d.emoji} {d.name}{!bewerk && (() => {
                            // Wie dronk wat — ook bij "gewoon rondjes", want ook daar kan je
                            // toewijzen. Amber, kroontje bij je eigen naam, onbenoemd in het rood.
                            const alleWie = people.filter((p) => (r.orders[d.id]?.[p.id] ?? 0) > 0)
                            ...
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
                              <button style={{ width: 30, height: 30, borderRadius: 8, background: "#eef1f6", border: "1px solid rgba(29,41,66,0.2)", fontSize: 18, color: "#6b7484", fontWeight: 800, cursor: "pointer" }}
                                onClick={(e) => {
                                  e.stopPropagation()
                                  if (val <= 0) return
                                  // "weg" onthoudt per drankje bij wie elke aftrek vandaan komt: een lege
                                  // string is het onbenoemde restant, anders het id van de gast. Zo weet
                                  // saveEditRound het precies en draait Annuleren ook de keuze terug.
                                  const omlaag = (pid: string) => setEditDraft((c) => c ? {
                                    ...c,
                                    drinks: { ...c.drinks, [d.id]: Math.max(0, (c.drinks[d.id] ?? n) - 1) },
                                    weg: { ...c.weg, [d.id]: [...(c.weg[d.id] ?? []), pid] },
                                  } : c)
                                  // Eerst het onbenoemde restant opmaken; pas daarna raak je iemands naam.
                                  const alWeg = dr?.weg[d.id] ?? []
                                  const anonOver = (r.anon[d.id] ?? 0) - alWeg.filter((x) => x === "").length
                                  if (anonOver > 0) { omlaag(""); return }
                                  const opties = people
                                    .map((pp) => ({ id: pp.id, naam: pp.name, n: (r.orders[d.id]?.[pp.id] ?? 0) - alWeg.filter((x) => x === pp.id).length }))
                                    .filter((x) => x.n > 0)
                                  if (opties.length === 0) { omlaag(""); return }
                                  if (opties.length === 1) {
                                    setConfirmDlg({ variant: "danger", msg: L.removeAssignedQ(d.name, opties[0].naam), yes: L.removeWord, onYes: () => { setConfirmDlg(null); omlaag(opties[0].id) } })
                                    return
                                  }
                                  // Meer dan één gast heeft dit drankje: dan mag jij zeggen bij wie.
                                  setKiesWeg({ did: d.id, naam: d.name, opties })
                                }}>−</button>
                              <span style={{ fontSize: 19, fontWeight: 800, color: "#c98a00", minWidth: 28, textAlign: "center" }}>{val}×</span>
                              <button style={{ width: 30, height: 30, borderRadius: 8, background: AAN, border: "none", fontSize: 18, color: "#fff", fontWeight: 800, cursor: "pointer" }}
                                onClick={(e) => { e.stopPropagation(); setEditDraft((c) => {
                                  if (!c) return c
                                  // Stond er nog een aftrek open, dan neem je die terug in plaats van er
                                  // een nieuw drankje bovenop te zetten — anders klopt "weg" niet meer.
                                  const lijst = c.weg[d.id] ?? []
                                  return { ...c,
                                    drinks: { ...c.drinks, [d.id]: (c.drinks[d.id] ?? n) + 1 },
                                    weg: lijst.length > 0 ? { ...c.weg, [d.id]: lijst.slice(0, -1) } : c.weg }
                                }) }}>+</button>
                            </span>
                          ) : (
                      ...
                      {!bewerk ? (
                        ...
                        // bijzetten, niet het hele rondje opnieuw hoeven doen.
                        <div style={{ marginTop: 10 }}>
                          {!addDrinkOpen ? (
                            <span onClick={(e) => { e.stopPropagation(); setAddDrinkCat(null); setAddDrinkZoek(""); setAddDrinkOpen(true) }}
                              style={{ display: "inline-block", fontSize: 14.5, fontWeight: 800, color: "#8a5e0f", background: "#fffdf4", border: "1.5px solid rgba(240,165,0,0.6)", borderRadius: 999, padding: "7px 14px", cursor: "pointer" }}>{L.addDrinkBtn}</span>
                          ) : (
                            <div style={{ background: "#eef1f6", borderRadius: 12, padding: 10 }}>
                              <div style={{ ...S.row, justifyContent: "space-between", marginBottom: 8 }}>
                              ...
                              {(() => {
                                const kies = (d: Drink) => { setEditDraft((c) => c ? { ...c, drinks: { ...c.drinks, [d.id]: (c.drinks[d.id] ?? 0) + 1 } } : c); setAddDrinkOpen(false) }
                                const pil = (d: Drink) => (
                                  <span key={d.id} onClick={(e) => { e.stopPropagation(); kies(d) }}
                                ...
                                if (resterend.length === 0) return <span style={{ fontSize: 14, color: "#6b7484" }}>{L.allDrinksInRound}</span>
                                const zoek = addDrinkZoek.trim().toLowerCase()
                                if (zoek) {
                                  return <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{resterend.filter((d) => d.name.toLowerCase().includes(zoek)).map(pil)}</div>
                                }
                                // Zonder zoekterm eerst alleen de categorieën, onder elkaar. Tik er
                                // één aan en de drankjes klappen eronder open; nog eens tikken
                                // klapt ze weer dicht. Zo geen lange muur van alle drankjes.
                                return (
                                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                                    {[...new Set(resterend.map((d) => d.cat))].map((c) => {
                                      const open = addDrinkCat === c
                                      const inCat = resterend.filter((d) => d.cat === c)
                                      return (
                                        <div key={c} style={{ background: "#fff", borderRadius: 10, border: `1px solid ${open ? "#1d2942" : "rgba(29,41,66,0.18)"}` }}>
                                          <div onClick={(e) => { e.stopPropagation(); setAddDrinkCat(open ? null : c) }}
                                          ...
                                          {open && <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: "0 10px 10px" }}>{inCat.map(pil)}</div>}
                                        </div>
                                      )
                                    })}
                                  </div>
                      ...
                      <div style={{ ...S.row, justifyContent: "space-between", alignItems: "center", marginTop: 11, paddingTop: 10, borderTop: "1px solid rgba(29,41,66,0.12)" }}>
                        ...
                        <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <input autoFocus={bedragFocus} onFocus={() => setBedragFocus(false)} onClick={(e) => e.stopPropagation()} type="text" inputMode="decimal" placeholder="0,00"
                            ...
                            style={{ ...S.input, width: 106, padding: "8px 10px", fontSize: 18, fontWeight: 800, color: uitPot ? "#2f5693" : "#c88a1a", textAlign: "right", borderColor: uitPot ? "rgba(47,111,181,0.45)" : undefined }} />
                        </span>
                      </div>
                    )}
                    {/* Wat het kostte en wie het voorschoot, in één blok. De potpil hieronder
                        zegt al of er pot-geld in zat — een aparte vraag "waarmee betaald?"
                        met zelf/pot/gemengd vroeg hetzelfde nog eens. */}
                    <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid rgba(29,41,66,0.12)" }}>
                      {!bewerk && (<>
                        <div style={{ display: (r.amount || 0) > 0.005 ? "flex" : "none", justifyContent: "space-between", alignItems: "center" }}>
                          ...
                          <span onClick={(e) => { e.stopPropagation(); startEditRound(r) }}
                            style={{ fontSize: 19, fontWeight: 600, color: "#c98a00", cursor: "pointer" }}>{(r.amount || 0) > 0 ? euro(r.amount) : "—"}</span>
                        </div>
                        {uitPot && (
        ...
        {fillMode ? (
          ...
          <button style={{ ...S.btnP, width: "100%", marginTop: 16, padding: "14px 6px", fontSize: 17.5 }}
            onClick={() => {
              ...
              if (fromQuick && rounds.length > 0) {
                setFillMode(false); setView("fairAssign"); return
              }
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {/* In Neem zelf op zit "zelfde rondje opnieuw" nu in de popup van Nieuw rondje. */}
              {settle && rounds.length > 0 && laatsteRondjeKlaar() && !tabsHier && (
  ...
  // ── SPLITTEN, STAP 2: wie dronk wat ─────────────────────────────────────────
  // Een eigen scherm in plaats van het toewijsvenster: elk rondje met zijn drankjes, en
  // onder elk drankje de gasten als pillen. Tik een naam aan en er gaat er één van het
  // drankje naar die gast; het minnetje in de pil geeft er één terug.
  if (view === "fairAssign") {
    const toonRondjes = rounds.map((r, idx) => ({ r, idx })).filter(({ r }) => drinks.some((d) => drinkTotalRound(r, d.id) > 0))
    const klaar = unassignedAllRounds === 0
    return (
      <div style={S.page}><div style={S.wrap}>
        <Header />
        {renderDialogs()}
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 15.5, color: "#6b7484", fontWeight: 600, lineHeight: 1.4 }}>{L.fairAssignSub}</div>
        </div>
        {toonRondjes.map(({ r, idx }) => {
          const roundDrinks = drinks.filter((d) => drinkTotalRound(r, d.id) > 0)
          ...
          return (
            <div key={r.id} style={{ ...S.card, position: "relative", padding: 0, overflow: "hidden",
              ...
              <div onClick={klap} style={{ ...S.row, justifyContent: "space-between", gap: 8, padding: "12px 14px", cursor: "pointer",
                background: los > 0 ? "rgba(224,104,92,0.06)" : "rgba(31,138,76,0.06)" }}>
                <span style={{ fontSize: 17.5, fontWeight: 800, color: "#1d2942", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{L.roundSummary(idx + 1, totaal)}</span>
                <span style={{ display: "inline-flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                  {los > 0
                    ? <span style={{ fontSize: 13.5, fontWeight: 800, borderRadius: 12, padding: "3px 9px", whiteSpace: "nowrap", color: "#b0402f", background: "rgba(224,104,92,0.14)" }}>{L.noNamePill(los)}</span>
                    : <span style={{ width: 24, height: 24, borderRadius: "50%", background: "#1f8a4c", color: "#fff", fontSize: 14, fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>✓</span>}
                  <span style={{ fontSize: 15, fontWeight: 800, color: "#6b7484" }}>{dicht ? "▾" : "▴"}</span>
                </span>
              </div>
              {!dicht && (
                <div style={{ padding: "2px 14px 12px" }}>
                  {roundDrinks.map((d, di) => {
                    const dlos = r.anon[d.id] ?? 0
                    return (
                      <div key={d.id} style={{ padding: "11px 0 3px", borderTop: di > 0 ? "1px solid rgba(29,41,66,0.1)" : "none" }}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                          {people.map((p) => {
                            const n = r.orders[d.id]?.[p.id] ?? 0
                            const op = dlos === 0 && n === 0
                            return (
                              <span key={p.id} role="button"
                                onClick={() => { if (dlos > 0) rAssignFromAnon(idx, d.id, p.id); else setNotice(L.drinkAllNamed) }}
                                style={{ ...S.chip(n), fontSize: 15, padding: n > 0 ? "5px 5px 5px 14px" : "9px 15px", opacity: op ? 0.45 : 1,
                                  ...(n === 0 && dlos > 0 ? { border: "1.5px dashed rgba(29,41,66,0.45)" } : {}) }}>
                                {opNaam === true && p.id === meId && <span style={{ display: "inline-flex", verticalAlign: "middle", marginRight: 4 }}><KroonIcoon size={14} kleur={n > 0 ? RANDTEKST : "#8a5e0f"} gevuld /></span>}
                                {p.name}
                                {n > 0 && <span style={S.badge}>{n}</span>}
                                {n > 0 && (
                                  <span role="button" aria-label="−" onClick={(e) => { e.stopPropagation(); rUnassign(idx, d.id, p.id) }}
                                    style={{ marginLeft: 6, display: "inline-flex", alignItems: "center", justifyContent: "center", width: 26, height: 26, borderRadius: "50%",
                                      background: "rgba(255,255,255,0.16)", color: "#fff", fontSize: 19, fontWeight: 800, lineHeight: 1, cursor: "pointer", flexShrink: 0 }}>−</span>
                                )}
                              </span>
                            )
                          })}
        ...
        <button disabled={!klaar}
          ...
          onClick={() => { if (klaar) setView("payers") }}>{L.toStep3}</button>
        {!klaar && (
          <div style={{ textAlign: "center", fontSize: 14, fontWeight: 800, color: "#b0402f", marginTop: 7 }}>{L.fairAssignLeft(unassignedAllRounds)}</div>
        )}
        {/* Terug is hier een volwaardige knop: je gaat terug naar de namen, en dat doe je
            vaak genoeg (iemand vergeten) om hem niet klein weg te stoppen. */}
        ...
        {rounds.map((r, idx) => {
          ...
          return (
            <div key={r.id} style={{ ...S.card, position: "relative", padding: "13px 14px", ...(geenBedrag
              ...
              <div style={{ ...S.row, justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                {/* Het bedrag vul je hier zelf in. Betalers die al gekozen zijn, delen
                    meteen mee in het nieuwe bedrag. */}
              ...
              {/* Welke drankjes erin zaten, op één regel: zo vind je het juiste bonnetje. */}
              <div style={{ fontSize: 14, color: "#8b93a3", fontWeight: 600, marginBottom: 10, paddingLeft: mist ? 0 : 20, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
