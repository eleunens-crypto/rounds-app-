...
export default function PartyTest() {
  ...
  const paymentState = () => {
    ...
    else if (nPayers === 0) { valid = false; reason = "Kies wie betaalde." }
    ...
    else if (multi && !allFilled) { valid = false; reason = "Vul een bedrag in per betaler — of tik ‘verdeel gelijk’." }
  ...
  const splitEqually = () => {
    const total = parseFloat(amountDraft.replace(",", ".")) || 0
    const n = payPersons.length + (payPot ? 1 : 0)
    if (total <= 0 || n === 0) return
    const potAvail = potAvailNow()
    let potShare = 0
    let rest = total
    let persons = [...payPersons]
    if (payPot) { potShare = Math.min(total / n, Math.max(0, potAvail)); rest = total - potShare }
    const per = persons.length ? rest / persons.length : 0
    const next: Record<string, string> = {}
    persons.forEach((pid) => (next[pid] = per.toFixed(2)))
    setPayAmts(next)
    setPotAmtDraft(payPot ? potShare.toFixed(2) : "")
    setPaidConfirmed(false)
  }
  const togglePayPerson = (pid: string) => { setPayPersons((ps) => ps.includes(pid) ? ps.filter((x) => x !== pid) : [...ps, pid]); setPaidConfirmed(false) }
  ...
  const nextRound = () => { if (blockIfUnpaid()) return; setRoundNr((n) => n + 1); setActiveCat(catsPresent[0]); setCupsChecked(false); setCupsTouched(false); setCart({}); setCartAnon({}); setView("order") }
    ...
    return (
      <div style={S.page}><div style={S.wrap}>
        ...
        <div style={S.card}>
          ...
          <>
          <div style={{ fontSize: 12.5, fontWeight: 800, color: "#8a7d55", marginBottom: 7 }}>Betaald door <span style={{ fontWeight: 600, color: "#b3a988" }}>(meerdere mogelijk)</span></div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
            <span style={{ ...S.chip(payPot ? 1 : 0), opacity: st.potAvail <= 0.005 ? 0.45 : 1 }} onClick={() => { if (!payPot && st.potAvail <= 0.005) { setNotice(`De ${potIsCard ? "drankkaart" : "pot"} is leeg (€0). Tik rechtsboven op “${potIsCard ? "drankkaart" : "pot"} + toevoegen” om eerst in te leggen.`); return } setPayPot((v) => !v); setPaidConfirmed(false) }}>{potIsCard ? "💳 drankkaart" : "🫙 de pot"}</span>
          ...
          {st.multi && (
            <div style={{ background: "#faf4e4", borderRadius: 12, padding: "10px 12px", marginTop: 10 }}>
              <div style={{ ...S.row, justifyContent: "flex-end", marginBottom: 8 }}>
                <span onClick={splitEqually} style={{ ...S.pill, cursor: "pointer", fontSize: 11.5, padding: "5px 10px", background: "rgba(31,138,76,0.14)", color: "#1f8a4c", fontWeight: 800 }}>👥 verdeel gelijk</span>
              </div>
              {payPot && (
        ...
        {rounds.map((r, idx) => {
          ...
          return (
            <div key={idx} style={{ ...S.card, padding: 0, overflow: "hidden" }}>
              <div style={{ cursor: "pointer", padding: 14 }} onClick={() => { if (allRoundsOpen) { setAllRoundsOpen(false); setOpenRound(idx) } else { setOpenRound(open ? null : idx) } setEditOpen(false); setEditAssign(false); setEditCups(false); setEditPay(false) }}>
                <div style={{ ...S.row, justifyContent: "space-between" }}>
              ...
              {open && (
                <div style={{ padding: "0 14px 14px" }}>
                  ...
                  <div style={{ ...S.row, justifyContent: "flex-end", marginTop: 10 }}>
                    <button style={{ ...S.btn, fontSize: 12, padding: "5px 12px", fontWeight: 800, color: "#8a5e0f" }} onClick={() => { const next = !editOpen; setEditOpen(next); if (!next) { setEditAssign(false); setEditCups(false); setEditPay(false) } }}>{editOpen ? "▴ sluiten" : "✏️ aanpassen"}</button>
                  </div>
                  {editOpen && (
                    <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                      <button style={{ ...S.btn, flex: 1, fontSize: 11.5, padding: "7px 0", ...(editAssign ? { background: "rgba(240,165,0,0.16)", borderColor: "rgba(240,165,0,0.5)", fontWeight: 800 } : {}) }} onClick={() => { setEditAssign((v) => !v); setEditCups(false); setEditPay(false) }}>toewijzen{!drinks.some((d) => (r.anon[d.id] ?? 0) > 0) && <span style={{ color: "#1f8a4c", fontWeight: 800 }}> ✓</span>}</button>
                      <button style={{ ...S.btn, flex: 1, fontSize: 11.5, padding: "7px 0", ...(editPay ? { background: "rgba(240,165,0,0.16)", borderColor: "rgba(240,165,0,0.5)", fontWeight: 800 } : {}) }} onClick={() => { setEditPay((v) => !v); setEditAssign(false); setEditCups(false) }}>bedrag &amp; betaler</button>
                      {depositOn && <button style={{ ...S.btn, flex: 1, fontSize: 11.5, padding: "7px 0", ...(editCups ? { background: "rgba(240,165,0,0.16)", borderColor: "rgba(240,165,0,0.5)", fontWeight: 800 } : {}) }} onClick={() => { setEditCups((v) => !v); setEditAssign(false); setEditPay(false) }}>bekers</button>}
                    </div>
                  )}
