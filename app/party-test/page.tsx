...
export default function PartyTest() {
  ...
  const [bekersPrompt, setBekersPrompt] = useState(false)
  const [coinsPrompt, setCoinsPrompt] = useState(false)
  const [coinsReturn, setCoinsReturn] = useState(false)
  const onbStepsRef = useRef<string[]>([])
  ...
  const closePot = () => {
    ...
    if (onbPotActive) {
      ...
      if (potChosen && willHave <= 0.005) {
        setConfirmDlg({ msg: `Je koos voor een ${potIsCard ? "drankkaart" : "pot"}, maar er is nog niks ingelegd. Toch zonder verder gaan? Je kan later nog toevoegen via de knop bovenaan.`, yes: `Toch verder zonder ${potIsCard ? "drankkaart" : "pot"}`, onYes: () => { setConfirmDlg(null); setPotChosen(false); advanceOnb() }, onNo: () => { setConfirmDlg(null); setShowPot(true); setOnbPotActive(true) } })
        return
      }
      advanceOnb()
    }
  ...
  const applyBeginChoices = () => {
    ...
    const q: string[] = []
    if (potOn) q.push("pot")
    if (bpBekers) q.push("bekers")
    if (bpCoins) q.push("coins")
    onbStepsRef.current = q
    advanceOnb()
  }
  const startOrdering = () => { setActiveCat(catsPresent[0]); setCupsChecked(false); setCupsTouched(false); setView("order") }
  const activateOnb = (step: string) => { if (step === "pot") { setShowPot(true); setOnbPotActive(true) } else if (step === "bekers") setBekersPrompt(true); else if (step === "coins") setCoinsPrompt(true) }
  const advanceOnb = () => { const q = onbStepsRef.current; if (!q || q.length === 0) { startOrdering(); return } const next = q[0]; onbStepsRef.current = q.slice(1); activateOnb(next) }
  const tryBegin = () => {
  ...
  if (view === "setup") {
    return (
      <div style={S.page} onClick={() => { setCoinInfo(false); setDepositInfo(false) }}><div style={S.wrap}>
        ...
        {bekersPrompt && (
          <div style={{ ...S.overlay, zIndex: 65 }} onClick={() => setBekersPrompt(false)}>
            <div style={S.sheet} onClick={(e) => e.stopPropagation()}>
              <h3 style={{ ...S.h3, fontSize: 18, marginTop: 0 }}>♻️ Herbruikbare bekers</h3>
              <p style={{ ...S.sub, marginBottom: 14 }}>Waarborg per beker die je terugkrijgt bij inleveren.</p>
              {pay === "coin" && (
                <div style={{ ...S.row, gap: 6, marginBottom: 10 }}>
                  <div style={{ ...S.seg(depositUnit === "coin"), padding: "7px 6px", fontSize: 12.5 }} onClick={() => setDepositUnit("coin")}>in coins</div>
                  <div style={{ ...S.seg(depositUnit === "eur"), padding: "7px 6px", fontSize: 12.5 }} onClick={() => setDepositUnit("eur")}>in €</div>
                </div>
              )}
              <div style={{ ...S.row, justifyContent: "space-between", marginBottom: 16 }}>
                ...
                <div style={{ ...S.row, gap: 4 }}>
                  {effDepositUnit === "eur" && <span style={{ fontSize: 14, fontWeight: 700, color: "#8a7d55" }}>€</span>}
              ...
              <button style={{ ...S.btnP, width: "100%" }} onClick={() => { if ((depositValue || 0) <= 0) { setNotice("Vul het waarborgbedrag in — of kies ‘toch niet met bekers’."); return } setDepositOn(true); setBekersPrompt(false); advanceOnb() }}>Verder →</button>
              <button style={{ background: "none", border: "none", width: "100%", marginTop: 10, fontSize: 12.5, color: "#c0554a", fontWeight: 700, cursor: "pointer", textDecoration: "underline" }} onClick={() => { setDepositOn(false); setBekersPrompt(false); advanceOnb() }}>✕ toch niet met bekers</button>
            </div>
          </div>
        )}
        {coinsPrompt && (
          <div style={{ ...S.overlay, zIndex: 65 }} onClick={() => setCoinsPrompt(false)}>
            <div style={S.sheet} onClick={(e) => e.stopPropagation()}>
              <h3 style={{ ...S.h3, fontSize: 18, marginTop: 0 }}>🎟️ Coins</h3>
              <p style={{ ...S.sub, marginBottom: 14 }}>Betaal je met coins i.p.v. euro's? Stel de coin-waarde in.</p>
              <div style={{ ...S.row, justifyContent: "space-between", marginBottom: 14 }}>
                <span style={{ fontSize: 14, fontWeight: 700 }}>1 coin =</span>
                <div style={{ ...S.row, gap: 4 }}><span style={{ fontSize: 14, color: "#8a7d55", fontWeight: 700 }}>€</span><input style={{ ...S.input, width: 70 }} type="text" inputMode="decimal" value={coinValue} onChange={(e) => setCoinValue(parseFloat(e.target.value.replace(",", ".")) || 0)} /></div>
              </div>
              <button style={{ ...S.btn, width: "100%", marginBottom: 12, fontSize: 12.5 }} onClick={() => { setCoinsReturn(true); setCoinsPrompt(false); setShowCoins(true); setView("settings") }}>🎟️ coin-prijzen per drankje aanpassen →</button>
              <div style={{ fontSize: 11.5, color: "#8a7d55", marginBottom: 14, lineHeight: 1.5 }}>💡 Prijzen per drankje staan al standaard ingevuld — aanpassen is optioneel.</div>
              <button style={{ ...S.btnP, width: "100%" }} onClick={() => { if ((coinValue || 0) <= 0) { setNotice("Vul de coin-waarde in — of kies ‘toch niet met coins’."); return } setPay("coin"); setCoinsPrompt(false); advanceOnb() }}>Verder →</button>
              <button style={{ background: "none", border: "none", width: "100%", marginTop: 10, fontSize: 12.5, color: "#c0554a", fontWeight: 700, cursor: "pointer", textDecoration: "underline" }} onClick={() => { setPay("eur"); setDepositUnit("eur"); setCoinsPrompt(false); advanceOnb() }}>✕ toch niet met coins</button>
            </div>
          </div>
        )}
        <div style={S.card}>
  ...
  if (view === "settings") {
    return (
      <div style={S.page} onClick={() => { setCoinInfo(false); setDepositInfo(false) }}><div style={S.wrap}>
        ...
        {coinsReturn && (
          <div style={{ background: "rgba(240,165,0,0.12)", border: "1px solid rgba(240,165,0,0.5)", borderRadius: 12, padding: "10px 12px", marginBottom: 12 }}>
            <div style={{ fontSize: 12.5, color: "#8a5e0f", fontWeight: 700, marginBottom: 8 }}>Pas hier de coin-prijzen aan. Klaar? Ga terug om verder te gaan.</div>
            <button style={{ ...S.btnP, width: "100%", padding: "9px 0" }} onClick={() => { setCoinsReturn(false); setView("setup"); setCoinsPrompt(true) }}>← terug naar coins</button>
          </div>
        )}
