...
// Hoe lang elke stap in de uitgeklapte lijst oplicht (estafette).
const STAP_MS = 1600
...
export default function Home() {
  ...
  // Estafette in de uitgeklapte lijst: de stappen lichten één voor één op. Er staat
  // maar één kaart open, dus er beweegt nooit op twee plekken tegelijk. Dichtgeklapt
  // beweegt er niets meer. Bij reduced motion staan alle stappen gewoon helder.
  const [stapNu, setStapNu] = useState(0)
  const [stilstaan, setStilstaan] = useState(false)
  useEffect(() => {
    try { setStilstaan(window.matchMedia("(prefers-reduced-motion: reduce)").matches) } catch { /* niets */ }
  }, [])
  useEffect(() => {
    setStapNu(0)
    if (!uitgeklapt || stilstaan) return
    const aantal = (uitgeklapt === "table" ? t.tableFlow : t.partyFlow).length
    const id = window.setInterval(() => {
      if (document.hidden) return
      setStapNu((s) => (s + 1) % aantal)
    }, STAP_MS)
    return () => window.clearInterval(id)
  }, [uitgeklapt, stilstaan, t])
  ...
  // "toon meer" op de scheidingslijn vlak onder de ondertitel — groter dan vroeger en
  // in de accentkleur, zodat hij opvalt. Hij staat bewust ver van de startknop: onder
  // de pil zit nog een ruime witruimte (zie paddingBottom van de kaarttekst), zodat je
  // nooit per ongeluk start terwijl je uitleg wou.
  const toonMeer = (m: Mode) => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 14 }}>
        ...
        <button type="button" aria-expanded={open} onClick={(e) => { e.stopPropagation(); klapUit(m) }} className="rundo-meer"
          style={{ flexShrink: 0, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit",
            fontSize: 16, fontWeight: 800, padding: "10px 22px", borderRadius: 999,
            border: `1.5px solid ${kleur}99`, background: "rgba(0,0,0,0.3)", color: kleur }}>
  ...
  // klein "of" tussen en delen ze één nummer. Estafette: de actieve stap licht op,
  // de andere zakken wat weg.
            ...
            <div key={i} style={{ position: "relative", display: "flex", alignItems: "center", gap: 9, padding: "5px 0",
              ...
              {st.iconen.map((ic, k) => (
                <span key={k} style={{ display: "flex", alignItems: "center", gap: 9 }}>
                  ...
                  <span style={{ flexShrink: 0, width: 32, height: 32, borderRadius: "50%",
                    background: loopt ? `${kleur}66` : `${kleur}2e`,
                    border: `1px solid ${kleur}80`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16,
                    transition: "background .3s ease" }}>{ic}</span>
                </span>
              ))}
  ...
  // Dichtgeklapt krijgt de kaarttekst onderaan extra lucht, zodat "toon meer" en de
  // startknop ver genoeg uit elkaar liggen. Opengeklapt staan de stappen ertussen.
  const bodyOnder = (m: Mode) => (uitgeklapt === m ? 18 : 30)
    ...
    <div style={S.page}>
      <div style={{ maxWidth: 360, margin: "0 auto", paddingTop: "max(18px, env(safe-area-inset-top))", paddingBottom: 28 }}>
        ...
        <div onClick={() => klapUit("table")} style={{ ...S.modeCard, ...S.tableCard, ...cardState("table") }} className="rundo-card rundo-card-table">
          ...
          <div style={{ ...S.cardBody, paddingBottom: bodyOnder("table"), maxWidth: uitgeklapt === "table" ? "100%" : "74%" }}>
        ...
        <div onClick={() => klapUit("party")} style={{ ...S.modeCard, ...S.partyCard, ...cardState("party") }} className="rundo-card rundo-card-party">
          ...
          <div style={{ ...S.cardBody, paddingBottom: bodyOnder("party"), maxWidth: uitgeklapt === "party" ? "100%" : "74%" }}>
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* "toon meer": krimpt heel even bij het indrukken, zodat je voelt dat je hem raakte. */
        .rundo-meer { transition: transform .1s ease, filter .15s ease; }
        .rundo-meer:active { transform: scale(0.95); }
        .rundo-meer:focus-visible { outline: 2px solid #fff; outline-offset: 2px; }
        @media (hover: hover) {
          .rundo-start-party:hover { background: rgba(240,193,75,0.18); }
          .rundo-start-table:hover { background: rgba(91,159,214,0.18); }
          .rundo-meer:hover { filter: brightness(1.25); }
        }
      `}</style>
    </div>
  )
